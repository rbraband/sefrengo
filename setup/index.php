<?php
// File: $Id: index.php 690 2012-04-16 00:21:31Z bjoern $
// +----------------------------------------------------------------------+
...
class setup {
	...
	var $updatefiles = array(
                            ...
                            'updates_from.01.09.91.sql',
                            'updates_from.01.09.92.sql',
	);
	...
	*/
	function setup()
	{
		...
		//manipulate some actions if user chose update
		if (isset($this -> globals['mode']) && $this -> globals['mode'] == 'update') {
	...
	*/
	function make_setup()
	{
		if (isset($this -> globals['action']) && $this -> globals['action'] != '') {
	...
	*/
	function screen_pretest()
	{
        ...
        // PHP Version		
		$tpl -> insert('test_PHP_check', 'name_PHP_check' , 'PHP version >= 5.3.0' );
		$tpl -> insert('test_PHP_check', 'value_PHP_check', $this -> test_config( version_compare(phpversion(), '5.3.0') <= 0?0:1  ));
		$tpl -> insert('test_PHP_check', 'class_PHP_check', $this -> test_config_text(version_compare(phpversion(), '5.3.0') <= 0?0:1 ));
		$tpl -> insert('test_PHP_check', 'style_PHP_config', $this -> test_config_style(version_compare(phpversion(), '5.3.0') <= 0?0:1 ));
	...
	function manage_insert_sql_dump(){
		if($this->globals['sql_target'] == 'updates.sql'){
			...
			for($i=$key; $i<$filecount; $i++){
				...
				//global sql update
				$return_this .= $this->insert_sql_dump(array('cms_path' => $this->globals['root_path'], 'cms_full_http_path' => $this->globals['root_full_http_path']));
				...
				if( is_file('sql/' .$basefile . '_client.sql') ) {
					...
					if (!$result) {
						die("Failed in get_table_content - SELECT idclient FROM ".$this->globals['prefix']."clients");
					}
					...
					foreach ($idclients AS $k=>$v) {
						$return_this .= $this->insert_sql_dump(array('idclient'=>$v, 'cms_path' => $this->globals['root_path'], 'cms_full_http_path' => $this->globals['root_full_http_path']));
					}
				...
				if( is_file('sql/' .$basefile . '_lang.sql') ) {
					...
					$replacements = array();
					while ($row = mysql_fetch_array($result)) {
						$replacements[ $row['idlang'] ] = array('idclient'=>$row['idclient'], 'idlang'=>$row['idlang'], 'cms_path' => $this->globals['root_path'], 'cms_full_http_path' => $this->globals['root_full_http_path']);
					}
					mysql_free_result($result);
	...
	function remove_remarks($sql) {
		...
		for ($i = 0; $i < $linecount; $i++) {
			if (($i != ($linecount - 1)) || (strlen($lines[$i]) > 0)) {
				if ($lines[$i]['0'] != '#' && ($lines[$i]['0'] != '-' && isset($lines[$i]['1']) && $lines[$i]['1'] != '-') ) {
					 $output .= $lines[$i] . "\n";
				} else $output .= "\n";
