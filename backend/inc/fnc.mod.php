<?PHP
// File: $Id: fnc.mod.php 425 2011-07-31 16:10:34Z holger $
// +----------------------------------------------------------------------+
...
function mod_copy($idmod, $from, $into) {
    global $db, $auth, $cms_db, $cfg_cms, $rep, $mod, $perm, $cms_log;
    if (!$from) $from = '0';
    if (!$into) $into = '0';
    ...
    if (is_array($modul)) {
        ...
        if ($from != $into) {
            ...
            if ($uninstall != '' && $is_install) {
                ...
                // Event
                fire_event('mod_uninstall_sql', array('idmod' => $idmod, 'name' => $name, 'uninstall_sql' => $uninstall));
                $return = '0407';
        ...
        if ($installer) {
            ...
            // Event
            fire_event('mod_install_sql', array('idmod' => $idmod, 'name' => $name, 'install_sql' => $install));
            return '0409';
        }
        // Event
        if ($from != '0') {
			$eventname = 'mod_export';
            $return = '0414';
        } else {
			$eventname = 'mod_import';
            $return = '0402';
        }
		//Log
		$cms_log->info('user', $eventname, array('idmod' => $idmod, 'name' => $name, 'version' => $version, 'verbose' => $verbose));
		//Event
...
function mod_set_config_all($idmod, $config = '') {
    // Konfiguration in alle Templatekonfigurationen �bernehmen?
    global $db, $cms_db;
    if ($idmod >= 1) {
...
function mod_save ($idmod_in, $name, $verbose, $description, $modversion, $modcat, $input, $output, $idclient, $repid = '', $sql_install = '', $sql_uninstall = '', $sql_update = '', $mod_rebuild_sql = false, $source_id = '0', $mod_no_wedding = false, $stripe = false, $mod_config_takeover = false) {
    ...
    if (!$idmod) {
		...
		//Log
		$cms_log->info('user', 'mod_new', array('idmod' => $idmod, 'name' => $name, 'version' => $modversion, 'verbose' => $modverbose));
		// Event
        fire_event('mod_new', array(
			'idmod' => $idmod,
			'name' => $name,
			'description' => $description,
			'version' => $modversion,
	...
	if ($output != $output_old) {
		...
		//Log
		$cms_log->info('user', 'mod_edit', array('idmod' => $idmod, 'name' => $name, 'version' => $modversion, 'verbose' => $modverbose));
        // Event
        fire_event('mod_edit', array(
			'idmod' => $idmod,
			'name' => $name,
			'description' => $description,
			'version' => $modversion,
    ...
    if ($idclient > 0 && $mod_sql_install != '' && $mod_rebuild_sql == true) {
        ...
        // Event
        fire_event('mod_install_sql', array('idmod' => $idmod, 'name' => $name, 'install_sql' => $sql_install));
    }
    if ($change) {
        // Status der 'code' Tabelle �ndern
        $list = get_idtplconf_by_using_type($idmod, 'mod');
...
function mod_download($idmod, $idclient) {
    ...
    if (is_array($modul)) {
		...
		//Log
		$cms_log->info('user', 'mod_download', array('idmod' => $modul['idmod'], 'name' => $modul['name'], 'version' => $modul['version']));
        // Event
        fire_event('mod_download', $modul);
        ob_end_clean();
...
function mod_upload($idclient, $override = false, $module = array(NULL), $cmodule = array(NULL), $smodule = array(NULL), $rmodule = array(NULL)) {
    global $cfg_cms, $db, $cms_db, $rep, $perm, $auth, $sess, $s_upload, $cms_log;
    ...
    if ($modul['install_sql'] != '' && $idclient >= 1 && $repcount < 1) {
        ...
        fire_event('mod_install_sql', array(
			'idmod' => $idmod,
			'name' => $modul['name'],
			'install_sql' => $modul['install_sql']));
        $return = '0409';
    } else $return = '0408';
    lib_delete_file($tmp_file['copy']);
	//Log
	$cms_log->info('user', 'mod_upload', array('idmod' => $modul['idmod'], 'name' => $modul['name'], 'version' => $modul['version']));
    // Event
    fire_event('mod_upload', $modul);
    return $return;
}
// Repository && Local
function mod_update($idmod, $repid, $modname, $description, $modversion, $modcat, $input, $output, $sql_install, $sql_uninstall, $sql_update, $idclient, $force = false) {
    ...
    if ($idclient > 0 && $sql_update != '') {
        ...
        // Event
        fire_event('mod_repository_update_sql', array(
			'idmod' => $idmod,
			'repository_id' => $repid,
			'name' => $modname,
			'update_sql' => $sql_update
	...
	//Log
	$cms_log->info('user', 'mod_repository_update', array('idmod' => $idmod, 'name' => $modname, 'version' => $modversion));
    // Event
    fire_event('mod_repository_update', array(
		'idmod' => $idmod,
		'name' => $modname,
		'description' => $description,
		'version' => $modversion,
...
function mod_lupdate($idmod, $repid, $modname, $description, $modversion, $modcat, $input, $output, $sql_install, $sql_uninstall, $sql_update, $idclient, $force = false) {
    ...
    if ($idclient > 0 && $sql_update != '') {
        $error = $rep->bulk_sql($sql_update);
        // Event
        fire_event('mod_local_update_sql', array(
			'idmod' => $idmod,
			'name' => $modname,
			'update_sql' => $sql_update
		));
	...
	//Log
	$cms_log->info('user', 'mod_local_update', array('idmod' => $idmod, 'name' => $modname, 'version' => $modversion));
    // Event
    fire_event('mod_local_update', array(
		'idmod' => $idmod,
		'name' => $modname,
		'description' => $description,
		'version' => $modversion,
...
function mod_install($repid, $modname, $description, $modversion, $modcat, $input, $output, $sql_install, $sql_uninstall, $sql_update, $idclient, $force = false) {
    ...
    if ($idclient > 0 && $sql_install != '') {
        ...
        // Event
        fire_event('mod_repository_install_sql', array(
			'repository_id' => $repid,
			'name' => $modname,
			'install_sql' => $sql_install
		));
    ...
    // Event
    fire_event('mod_repository_import', array(
		'name' => $modname,
		'description' => $description,
		'version' => $modversion,
		'cat' => $modcat,
