<?PHP
// File: $Id: fnc.tpl.php 600 2011-12-19 01:13:48Z bjoern $
// +----------------------------------------------------------------------+
...
function tpl_save($idtpl, $idlay, $tplname, $description, $tpl_overwrite_all) {
	global $db, $auth, $client, $cms_db, $cms_lang, $cfg_client, $tpl, $perm, $cms_log;
	...
	if (!$idtpl) {
		...
		//Log
		$cms_log->info('user', 'tpl_new', array('idtpl' => $idtpl, 'name' => $tplname));
		// Event
		fire_event('tpl_new', array(
			'idtpl' => $idtpl,
			'idlay' => $idlay,
			'name' => $tplname,
			'description' => $description,
			'overwrite_all' => (($tpl_overwrite_all != '1') ? 0 : 1),
		...
		//Log
		$cms_log->info('user', 'tpl_edit', array('idtpl' => $idtpl, 'name' => $tplname));
		// Event
		fire_event('tpl_edit', array(
			'idtpl' => $idtpl,
			'idlay' => $idlay,
			'name' => $tplname,
			'description' => $description,
	...
	if ($change) {
		...
		// l�sche alte 'content' Eintr�ge
		if (is_array($empty_container)) {
			$list2 = get_idsidelang_by_idtplconf($list);
			//wenn keine seiten mit template xyz ermittelt werden kann, kann/darf auch nichts gel�scht werden
		...
		// jb_todo: change_code_status im dateimanager einf�hren
...
function tpl_delete_template($idtpl) {
	...
	if ($template_is_in_use) {
		...
		//Daten vor dem Löschen für das Event laden
		$sql = "SELECT * FROM $cms_db[tpl] WHERE idtpl='$idtpl'";
		$db->query($sql);
		if ($db->next_record()) {
			$name = make_string_dump($db->f('name'));
			$description = make_string_dump($db->f('description'));
			$idlay = make_string_dump($db->f('idlay'));
			$is_start = make_string_dump($db->f('is_start'));
		...
		//Log
		$cms_log->info('user', 'tpl_delete', array('idtpl' => $idtpl, 'name' => $name));
		// Event
		fire_event('tpl_delete', array(
			'idtpl' => $idtpl,
			'idlay' => $idlay,
			'name' => $name,
			'description' => $description,
...
function con_config_tpl_save($idtpl, $idlay, $idcatlang, $idsidelang, $idtplconf, $have_perm_save_configdata = true) {
	...
	//Ein neues Template wird angelegt - recht des benutzers ist egal
		...
		//idlay ist bei zu wenig rechten nicht vorhanden, pr�fen ob neues Template
	...
	foreach ($container['id'] as $value) {
		if ($value != '0') {
			...
			// ge�nderte Container updaten
			if (in_array($value,$used_container['id'])) {
				// Modulkonfiguration hat sich ge�ndert
				...
				// Modul hat sich durch Templatewechsel ge�ndert
				if ($idtpl != $configtpl) {
			        $sql = "SELECT idcontainer FROM $cms_db[container] WHERE idtpl='$idtpl' AND container='$value'";
					$db->query($sql);
					$db->next_record();
...
function con_config_folder_save($idcat, $idcatside, $idtpl, $view, $idtplconf, $description, $name,  $rewrite_use_automatic, $rewrite_alias, 
	...
	global $db, $client, $lang, $cms_db, $auth, $cms_lang, $perm, $sess, $cfg_client, $cms_log;
	...
	// Im Frontend mu� ein Template angegeben werden
	...
	if (empty($idcat)) {
		...
		//onlinestaus �bergeordnete idcat holen
		$parent_visibility = 0;
		...
		// idcatlang f�r die Templateerstellung raussuchen
		...
		// Log
		$cms_log->info('user', 'con_cat_new', array('idcat' => $idcat, 'catname' => $name));
		// Event
		fire_event('con_cat_new', array(
			'idcat' => $idcat,
			'idtpl' => $idtpl,
			'idlay' => $idlay,
			'name' => $name,
			'description' => $description,
			'rewrite_use_automatic' => $rewrite_use_automatic,
        ...
        sf_factoryCallMethod('UTILS', 'DbCache', null, null, 'flushByGroup', array('frontend', 'content'));
		// Danach ins Frontend?
		if ($view) $url_location = $sess->url($cfg_client['path_http_edit'] . $cfg_client['path_rel'].$cfg_client['contentfile'].'?lang='.$lang.'&idcat='.$idcat.'&idcatside='.$idcatside.'&view='.$view);
	...
	// Ordner aktualisieren
		...
		// Log
		$cms_log->info('user', 'con_cat_edit', array('idcat' => $idcat, 'catname' => $name));
		// Event
		fire_event('con_cat_edit', array(
			'idcat' => $idcat,
			'idtpl' => $idtpl,
			'idlay' => $idlay,
			'name' => $name,
		...
		// Danach ins Frontend?
		if ($view) {
			$url_location = $sess->url($cfg_client['path_http_edit'] . $cfg_client['path_rel'].$cfg_client['contentfile'].'?lang='.$lang.'&idcat='.$idcat.'&idcatside='.$idcatside.'&view='.$view);
		} else {
			$url_location = $sess->url("main.php?area=con");
		}
	}

	// Cache-Group Frontend l�schen
...
function con_config_side_save($idcat, $idside, $idtpl, $idtplconf, $idsidelang, $idcatside, $idcatnew
	...
	global $db, $client, $sess, $perm, $lang, $cms_db, $cfg_client, $cms_lang, $cms_log;//, $val_ct;
	...
	$meta_is_https = ((int) $meta_is_https == 1) ? 1:0;
	...
	if (empty($idside)) {
		...
		foreach ($a_languages as $tmp_lang) {
			...
			$sql .= ' user_protected, online, start, end, meta_author, meta_description, meta_robots, meta_redirect_time, is_https, rewrite_use_automatic, rewrite_url) ';
			$sql .= 'VALUES (';
			...
			$sql .= " '$meta_author', '$tmp_meta_description', '$tmp_meta_robots', '$meta_redirect_time', '$meta_is_https', '$rewrite_use_automatic', '$rewrite_url')";
			$db->query($sql);
		}

		// idsidelang f�r die Templateerstellung raussuchen
		...
		// Log
		$cms_log->info('user', 'con_side_new', array('idside' => $idside, 'pagename' => $title));
		// Event
		fire_event('con_side_new', array(
			'idside' => $idside,
			...
			'meta_redirect_time' => $meta_redirect_time,
			'meta_redirect_url' => $meta_redirect_url,
			'meta_is_https' => $meta_is_https,
			'online' => $online,
			'startdate' => $startdate,
			'starttime' => $starttime,
			'enddate' => $enddate,
		...
		if ($view) $url_location = $sess->url($cfg_client['path_http_edit'] . $cfg_client['path_rel'].$cfg_client['contentfile'].'?lang='.$lang.'&idcatside='.$idcatside['0'].'&view='.$view);
		...
		$sql .= " meta_redirect_time = '$meta_redirect_time', is_https = '$meta_is_https', rewrite_use_automatic = '$rewrite_use_automatic', rewrite_url = '$rewrite_url' ";
		...
		if (is_array($tmp_idcat)) {
			...
			// Seite aus nicht benutzen Ordnern l�schen
			// jb_todo: rechte l�schen??
			foreach ($tmp_idcat as $value) {
				if (!in_array($value,$idcatnew)) {
					...
					// jb_todo:
					// l�sche alte 'tpl_conf' Eintr�ge
					// mu� noch eingetragen werden
...
//		}
		...
		// Log
		$cms_log->info('user', 'con_side_edit', array('idside' => $idside, 'pagename' => $title));
		// Event
		fire_event('con_side_edit', array(
			'idside' => $idside,
			'idsidelang' => $idsidelang,
			'idtpl' => $idtpl,
			'idlay' => $idlay,
        ...
        sf_factoryCallMethod('UTILS', 'DbCache', null, null, 'flushByGroup', array('frontend', 'content'));
		// ermittle redirect-url
		if ($view) $url_location = $sess->url($cfg_client['path_http_edit'] . $cfg_client['path_rel'].$cfg_client['contentfile'].'?lang='.$lang.'&idcatside='.$idcatside.'&view='.$view);
		else $url_location = $sess->url('main.php?area=con');
	}
...
function getIdList($sql, &$array_for_ids, $idfield, $valfield = '') {
	...
	// ermittle werte-feld und verhindere einen ung�ltigen namen
	$value_field = (empty($valfield)) ? $idfield:	$valfield;
	if (empty($value_field)) return 0;

	// f�hre sql aus und lade die gew�nschten werte ins array
	$db->query($sql);
	if (empty($idfield)) {
		// erstelle array durch anh�ngen der neuen werte
...
function createDate($dateval, $timeval) {
	...
	// zerlege �bergabetexte
	$arr_date = @explode('.', $dateval);
	$arr_time = @explode(':', $timeval);

	// pr�fe ob felder vorhanden, dann erzeuge datum
