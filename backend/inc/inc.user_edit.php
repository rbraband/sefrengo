<?PHP
// File: $Id: inc.user_edit.php 641 2012-02-18 02:33:32Z bjoern $
// +----------------------------------------------------------------------+
...
$tmp['LANG_LAST_LOGIN'] = $cms_lang['user_last_login'];
$tmp['LANG_LAST_LOGIN_FAILED'] = $cms_lang['user_last_invalid_login'];
$tmp['LANG_FAILED_COUNT'] = $cms_lang['user_failed_count'];
$tmp['LANG_LAST_MODIFIED'] = $cms_lang['user_last_modified'];

$tmp['LANG_DATA_LOGIN'] = $cms_lang['user_data_login'];
$tmp['LANG_DATA_PERSON'] = $cms_lang['user_data_person'];
$tmp['LANG_DATA_CONTACT'] = $cms_lang['user_data_contact'];
$tmp['LANG_DATA_FIRM'] = $cms_lang['user_data_firm'];
$tmp['LANG_DATA_MISC'] = $cms_lang['user_data_misc'];
$tmp['LANG_DATA_PROFILE'] = $cms_lang['user_data_profile'];
...
if (!isset($username) && !empty($iduser)) {
	...
	$tmp['FORM_LAST_LOGIN'] = ($sf_user->getCurrentLoginTimestamp() > 0) ? date($cfg_cms['format_date'], $sf_user->getCurrentLoginTimestamp()) . ' '.date($cfg_cms['format_time'], $sf_user->getCurrentLoginTimestamp()) : '-';
	$tmp['FORM_LAST_LOGIN_FAILED'] = ($sf_user->getLastLoginFailedTimestamp() > 0) ? date($cfg_cms['format_date'], $sf_user->getLastLoginFailedTimestamp()) . ' '.date($cfg_cms['format_time'], $sf_user->getLastLoginFailedTimestamp()):'-';
	...
	$tmp['FORM_LAST_MODIFIED'] = date($cfg_cms['format_date'], $sf_user->getLastmodifiedTimestamp()) . ' '.date($cfg_cms['format_time'], $sf_user->getLastmodifiedTimestamp()) .' von '.$sf_user2->getUsername();
