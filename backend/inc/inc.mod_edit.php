<?PHP
// File: $Id: inc.mod_edit.php 616 2012-01-11 00:09:29Z holger $
// +----------------------------------------------------------------------+
...
$code_attributes = $code_sql_attributes = '';
...
if($enable_code_editor == TRUE)
{
	...
	$options = array(
		//'value' => '',
		'mode' => 'application/x-httpd-php', // mode-name or MIME type
		//'theme' => 'default',
		//'indentUnit' => 2,
	...
	$options['mode'] = 'text/x-plsql'; // mode-name or MIME type
	$attributes['data-options'] = $options;
	$code_sql_attributes = $html->attributesArrayToHtmlString($attributes);
...
echo "    <table class=\"config\" cellspacing=\"1\">\n";
    ...
    if (!$noeditfield) {
        echo " <span style=\"padding-left:20px;\">".$cms_lang['gen_select']."</span> <select onchange=\"document.newmodule.modcat.value=(this.value);\" name=\"catchange\" maxlength=\"60\" size=\"1\">\n";
    ...
    echo "	     <td><textarea class=\"w800 sans-serif\" $noeditfield name=\"description\" rows=\"3\" cols=\"52\">" . htmlspecialchars($description, ENT_COMPAT, 'UTF-8') . "</textarea></td>\n";
    echo "	   </tr>\n";
    if ($editsql == '1') {
        echo "      <tr>\n";
        echo "        <td class=\"head\">" . $cms_lang["mod_sql_install"] . "</td>\n";
        echo "        <td><textarea ".$code_sql_attributes." class=\"codemirror w800\" name=\"install_sql\" id=\"install_sql\" rows=\"14\" cols=\"52\" wrap=\"off\">" . htmlspecialchars($sql_install, ENT_COMPAT, 'UTF-8') . "</textarea></td>\n";
        echo "	   </tr>\n";
        echo "      <tr>\n";
        echo "        <td class=\"head\">" . $cms_lang["mod_sql_uninstall"] . "</td>\n";
        echo "        <td><textarea ".$code_sql_attributes." class=\"codemirror w800\" name=\"uninstall_sql\" id=\"uninstall_sql\" rows=\"14\" cols=\"52\" wrap=\"off\">" . htmlspecialchars($sql_uninstall, ENT_COMPAT, 'UTF-8') . "</textarea></td>\n";
        echo "	   </tr>\n";
        echo "      <tr>\n";
        echo "        <td class=\"head\">" . $cms_lang["mod_sql_update"] . "</td>\n";
        echo "        <td><textarea ".$code_sql_attributes." class=\"codemirror w800\" name=\"update_sql\" id=\"update_sql\" rows=\"14\" cols=\"52\" wrap=\"off\">" . htmlspecialchars($sql_update, ENT_COMPAT, 'UTF-8') . "</textarea></td>\n";
        echo "	   </tr>\n"; 
        ...
        echo "        <td><textarea ".$code_attributes." class=\"codemirror w800\" name=\"input\" id=\"input\" rows=\"14\" cols=\"52\" wrap=\"off\">" . htmlspecialchars($input, ENT_COMPAT, 'UTF-8') . "</textarea></td>\n";
        echo "	   </tr>\n";
        echo "      <tr>\n";
        echo "        <td class=\"" . (($err_o) ? 'headerror' : 'head') . "\">" . $cms_lang["mod_output"] . "</td>\n";
        echo "        <td><textarea ".$code_attributes." class=\"codemirror w800\" name=\"output\" id=\"output\" rows=\"14\" cols=\"52\" wrap=\"off\">" . htmlspecialchars($output, ENT_COMPAT, 'UTF-8') . "</textarea></td>\n";
        echo "	   </tr>\n";
