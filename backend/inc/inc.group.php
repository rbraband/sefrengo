<?PHP
// File: $Id: inc.group.php 641 2012-02-18 02:33:32Z bjoern $
// +----------------------------------------------------------------------+
...
switch ($action) {
	case 'activate':
		...
		$cms_log->info('user', 'group_activate', array('idgroup' => $idgroup, 'name' => $groupname));
		fire_event('group_activate', array(
			'idgroup' => $idgroup,
			'name' => $groupname,
	...
	case 'deactivate':
		...
		fire_event('group_deactivate', array(
			'idgroup' => $idgroup,
			'name' => $groupname,
			'author' => $auth->auth['uid']	
		));
...
$tmp['NEW_GROUP'] = '<a href="'.sprintf($base_url, 'group_edit', $order, $ascdesc).'" class="action">'.$cms_lang['group_new'].'</a>';
