<?PHP
// File: $Id: fnc.general.php 632 2012-02-11 22:31:20Z bjoern $
// +----------------------------------------------------------------------+
...
//**********************************************
// HTTPS FUNCTIONS START
//**********************************************

/**
 * Returns the current serverprotocol
 * 
 * @return str 'http' or 'https' 
 */
function sf_get_server_protocol()
{
...
//**********************************************
// HTTPS FUNCTIONS STOP
//**********************************************

function fire_event($eventname, $args = array(), $returns = array())
{
	// initialized only onced, due to implementation as API singleton 
	$cms_event = sf_api('LIB', 'Event');
	return $cms_event->fire($eventname, $args, $returns);
}
...
//TODO REMOVE - function is moved to lib FrontendPageParser
...
function change_code_status($list, $status, $type='idcode') {
	...
	if ($list['0'] != '') {
		switch($type) {
			case 'idcode':
				$sql = "DELETE FROM $cms_db[code]  WHERE idcode IN(".implode(',', $list).")";
				break;

			case 'idcatside':
				$sql = "DELETE FROM  $cms_db[code]  WHERE idlang='$lang' AND idcatside IN(".implode(',', $list).")";
				break;
...
function get_idcode_by_idtplconf($list) {
	...
	if (!is_array($list) && $list != '') $list = explode(',',$list);
	if (is_array($list)) {
		$changed = "(C.changed='0' OR C.changed='2')";
		foreach ($list as $value) {
			...
			// das Template geh�rt einem Ordner
			if (!$db->affected_rows()) {
				$sql = "SELECT 
							idcode 
						FROM 
...
function make_array_to_urlstring($in) {
	...
	foreach($in as $key => $value) {
		if (is_array($value)) {
			// leere Arrayelemente l�schen und in String wandeln
...
function tree_level_order($node_id, $array, $expanded = 'false', $level = '0') {
	global $tlo_tree, ${$array}, ${$array.'_level'}, $perm, $con_tree;
...
// L�dt eine Validator-Klasse f�r �berpr�fungen beim CSS-Editor oder File-Uploads
function get_validator($name) {
	global $cfg_cms;

	// include validator class and create object
	if (!$cfg_cms[$name]) $cfg_cms[$name] = 'class.validator.php';
	include_once($cfg_cms['path_base'].$cfg_cms['path_backend_rel'].'inc/'.$cfg_cms[$name]);
