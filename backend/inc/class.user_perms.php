<?PHP
// File: $Id: class.user_perms.php 641 2012-02-18 02:33:32Z bjoern $
// +----------------------------------------------------------------------+
...
// + Autor: $Author: bjoern $
// +----------------------------------------------------------------------+
// + Revision: $Revision: 641 $
// +----------------------------------------------------------------------+
// + Description:
// +----------------------------------------------------------------------+
// | Authors: Bjï¿½rn Brockmann <bjoern@project-gooseberry.de>              |
// |          Jï¿½rgen Brï¿½ndle <braendle@web.de>                            |
// +----------------------------------------------------------------------+
// | Changed: 20.02.2004 - Jï¿½rgen Brï¿½ndle                                 |
// |          Erweiterung von get_rights_panel um Parent-Rechte           |
// |          23.02.2004 - Bjï¿½rn Brockmann                                |
// |          set_group_rights erweitert um check der 2. Parent Ebene     |
// |          empty(...) ersetzt gegens strlen($gruppenrights) < 1        |
// |          ï¿½nderungen sind mit //change bb markiert                    |
// |          24.02.2004 - Jï¿½rgen Brï¿½ndle                                 |
// |          Sperren des Rechtepanels, wenn keine konfigurierbaren Grup- |
// |          pen exitieren. Konstruktor liest hierfï¿½r die Anzahl der     |
// |          Gruppen aus und get_right_panel prï¿½ft diesen Wert           |
// |          get_right_fields zu private Methode _get_right_fields       |
// |          12.03.2004 - Jï¿½rgen Brï¿½ndle                                 |
// |          Areas ohne eigene Rechte subtituiert, betrifft zur Zeit nur |
// |          Area_SCAN und AREA_SCANCONTROL                              |
// |          12.04.2004 - Jï¿½rgen Brï¿½ndle                                 |
// |          Rechtepanel komplett in PHP erzeugen, wegen JS-Problemen in |
// |          Mozilla bei dynamisch geschriebenen Formularen              |
// |          24.04.2004 - Jï¿½rgen Brï¿½ndle                                 |
// |          Dokumentation, Minor Changes                                |
// |          25.04.2004 - Jï¿½rgen Brï¿½ndle                                 |
// |          Dokumentation, Rechtepanel um Zeile zum Setzen oder Lï¿½schen |
// |          aller Rechte einer Benutzergruppe ergï¿½nzt                   |
// |          02.05.2004 - Jï¿½rgen Brï¿½ndle                                 |
// |          Dokumentation, Minor Changes                                |
// |          10.05.2004 - Jï¿½rgen brï¿½ndle                                 |
// |          Neue Funktion: is_any_perm_set()                            |
// |          20.06.2006 - Bjï¿½rn Brockmann                                |
// |          Rechtevererbung, Addonobjekte hinzugefï¿½gt                   |
// +----------------------------------------------------------------------+
 ...
 * Klasse cms_perms
 *
 * Rechtemanagement fï¿½r Sefrengo
 *
 * Diese Klasse stellt die Funktionalitï¿½ten fï¿½r die Rechteverwaltung zur
 * Verfï¿½gung. Hierbei werden verschiedenste Funtkionen fï¿½r die Abfrage und
 * das Setzen von Rechten bereitgestellt.
 *
 * Das Rechtemanagement ist Rollenbasiert, benutzerbezogene Rechte werden
 * erst in spï¿½teren Versionen direkt unterstï¿½tzt.
 * Jeder Benutzer von Sefrengo kann einer oder mehreren Gruppen bzw.
 * Rollen zugeordnet werden. Hinzukommt, das alle Rechte sprachabhï¿½ngig fest-
 * gelegt werden, d.h. ein User kann in verschiedenen Sprache unterschiedliche
 * Rechte fï¿½r die Arbeit im Backend erhalten.
 *
 ...
 * zur Zeit nur in Ausnahmefï¿½llen statt, die bei den entsprechenden Bereichen
 * noch erlï¿½utert werden.
 *
 ...
 *
 * Fï¿½r alle Areas kann man festlegen, ob eine Benutzergruppe diesen Bereich betreten
 ...
 *
 * Unabhï¿½ngig vom Betreten eines Areas ist die Grundkonfiguration fï¿½r die Rechte
 * des Benutzers in einem Bereich. Diese kï¿½nnen so eingestellt werden, das der Benutzer
 ...
 * Ein Anwendungsfall hierfï¿½r ist das Bearbeiten von Seiten wo Elemente einer Seite
 ...
 * die sich in dem gewï¿½hlten Schema von max. 16 Main- und 16-Detail-Rechten unterbringen
 ...
 * Area Redaktion/Dateimanager. Main-Rechte setzen sich aus den Rechten zu den Funktionalitï¿½ten,
 * die in dieser Organisationsebene notwendig sind, und den Rechten, die auf der Detailebene
 * notwendig sind, zusammen. Diese Kombination ist fï¿½r die Vererbung der Rechte notwendig, da
 * die Main-Ebene festlegt, welche Funktionalitï¿½ten auf Elemente der Detailebene fï¿½r den
 * Benutzer erreichbar sind.
 *
 * Detail-Rechte beziehen sich - wie schon angedeutet - auf die Funktionalitï¿½ten der Elemente
 ...
 *
 * Die Vererbung der Rechte ermï¿½glicht eine schnelle und einfache Methode zur Rechteverwaltung,
 * die vorallem die Speicherung in der Datenbank minimiert. Es werden nur Rechte in der Datenbank
 * gespeichert, wenn sich Diffenrenzen zur Vererbung der ï¿½bergeordneten Rechte ergeben.
 * Durch die Vererbung ergibt sich auch eine klare Gewichtung im Rechtemanagement: Detailrechte
 * stehen ï¿½ber Mainrechten, die wiederum ï¿½ber Area-Rechten stehen. Vorausgesetzt die Rechte sind
 * in der Ebene oder am Element selbst anderst gesetzt als in der darï¿½berliegenden Ebene.
 *
 ...
 * immer von der ï¿½bergeordneten Rechte-Ebene erbt und nicht von der ï¿½bergeordneten Elemente-Ebene.
 ...
 * Redaktion/Seiten: Eine Kategorie erbt beim Anlegen die Rechte der ï¿½bergeordneten Elementebene,
 * d.h. die Rechte der Kategorie in der die neue Katagorie angelegt wird.
 * Redaktion/Dateimanager: Hier erbt ein Verzeichnis beim Anlegen die Rechte der ï¿½bergeordneten
 * Verzeichnisses.
 *
 * VORSICHT: Diese Aussage ist beschrï¿½nkt auf den Zeitpunkt des Anlegens einer Kategorie oder Ver-
 * zeichnisses. Nach dem Anlegen greift immer die vorher beschriebene Art der Vererbung bezogen auf
 * die Rechtehierarchie!
 * Zukï¿½nftige Anpassungen werden diese Unterschiede weiter umbauen zu einer Vererbung die sich auf
 * die Elementhierarchien abbildet.
 *
 * @author	Jï¿½rgen Brï¿½ndle, Bjï¿½rn Brockmann
 * @since	ALPHA
...
class cms_perms {
	...
	// Flag: User ist Adminstrator. Rï¿½ckgabe des Wertes durch die Funktion is_admin()
	var $_is_admin;
	...
	var $perm_addon;
	//Liste der Rechteabhï¿½ngigkeiten
	var $parent_rights = array();
	//Anzahl der angelegten Gruppen der CMS-Installation
	var $_group_count;
	//Objekte fï¿½r die Umgang mit der Datenbank und dem Debugging
	var $cms_db, $db, $cms_log;

	// Verschiedene Strings
	// Insert-SQL fï¿½r neuen Rechte-Eintrag
	var $insert_perm     = "INSERT INTO %s (idgroup, idlang, type, perm, id) VALUES (%s, %s, '%s', %s, '%s')";

	// Image fï¿½r den Aufruf des Rechtepanel
	 ...
	 * Erstellt das Rechte-Objekt fï¿½r die aktuelle Webanfrage
	 * Ermittelt die Rechtegruppen, Sprachen- und Projektzugehï¿½rigkeiten des angemeldeten Users
	 *
	 * @param	int		$client				Aktuelles Projekt
	 * @param	int		$lang				Aktuelle Sprache
	 * @param	bool	$simulate			Simulationsmodus  - Abfrage der Rechte fï¿½r die Rechtekonfiguration
	 *										optional - Standard: false
	 * @param	int|arr $simulate_idgroup	Simulationsgruppe - Gruppe fï¿½r die Rechtekonfiguration
	 *										optional - Standard: 0
	 *
	 * @global	$changeclient
	 * @global	$lang_charset
	...
	**/
	function cms_perms($client, $lang, $simulate = false, $simulate_idgroup = 0) {
		global $changeclient, $lang_charset, $auth;

		$this->_is_admin = false;
		$this->cms_db    = $GLOBALS['cms_db'];
		$this->cms_log   = $GLOBALS['cms_log'];
		$this->db        = new DB_cms;
		...
		// Benï¿½tigte Gruppeninformationen anhand der Userid extrahieren
		$this->get_group_infos_from_user();

		// Alle fï¿½r Gruppe mï¿½glichen Clients und Sprachen herausfinden
		$sql  = 'SELECT ';
		...
		if (empty($this->client) || empty($this->lang)) {
			$this->cms_log->warning('sf_core', 'No Client or lang found. Insert a new group? (file: '.__FILE__.')');
			return;
		}

		// Hole die Abhï¿½ngigkeiten der Zugriffsrechte und speichert diese als Array ab
		...
		$valcol->setIdclient($client = 0);
		$valcol->setIdlang($lang = 0);
		$valcol->getByGroupAndKeys('user_perms', array('key1' => 'cms_access'));
		$cms_perm_parents = $valcol->getAssocKeyArray();
		unset($valcol);
	...
	**/
	...
	/**
	 * Erstellt den HTML-Code fï¿½r die Anzeige einer Rechte-Popup-Ebene incl. des JavaScript-Aufrufs und der Buttons zum Aufruf.
	 *
	 * Die Werten der Gruppen, GruppenIDs und GruppenRechte fï¿½r ein Element werden mit der Methode _get_right_fields in
	 * Hidden-Fields eingetragen.
	 * Die Funktion holt die Rechte, die fï¿½r das Element festgelegt sind. Gibt es keine Eintragung wird die ï¿½bergeordnete Ebene
	 * ausgewertet, wenn der Parameter $parent_id ungleich 0 ist, ansonsten werden die Rechte des Areas verwendet.
	 *
	 * Gibt es keine konfigurierbaren Gruppen, wird nichts zurï¿½ckgegeben, andernfalls wird ein Array mit den notwendigen
	 * HTML-Konstrukten fï¿½r das Rechtepanel zurï¿½ckgeliefert.
	 *
	 * Da das HTML-Konstruct in PHP aufgebaut wird, ist die Ansicht nur ï¿½ber CSS-Stile anpassbar. Hierzu dienen die folgenden
	 ...
	 * #rightsmenu td.rightscellhead 						Zelle mit Spaltenï¿½berschriften im Rechtepanel
	 * #rightsmenu td.rightsnamehead 						Name der Spaltenï¿½berschriften
	 ...
	 *       "call"   => String mit dem JavaScript-Link fï¿½r den Aufruf des Rechte-Popups mittels popupmenu.js,
	 *       "scripts"=> String mit den HTML-Tags fï¿½r das Einbinden der Javascript-Funktionen und Klassen)
	 *
	 * @param	string	$type 			Rechtetype der benï¿½tigt wird
	 * @param	string	$id				ID des Elements, dessen Rechteeintrï¿½ge angezeigt werden sollen
	 * @param 	mixed	$config			Assoziatives Array mit Name-Werte-Paare fï¿½r die Konfiguration der Popup-Ebene oder
	 *									String mit dem Namen des Formulars in dem die Anzeige stattfindet
	 *									Optional - Standardwert fï¿½r Formularnamen: 'rights'
	 * @param	string	$view			Darstellung des Links zum Anzeigen des Popups, mï¿½gliche Werte: 'text' oder 'img'
	 *									Optional - Standardwert: text
	 * @param	bool	$ignore_user	Flag um die Anzeigen von Benutzerrechte auszuschlieï¿½en
	 *									Optional - Standard: false - keine Benutzerrechte einschlieï¿½en
	 * @param 	bool	$all_users		In Zusammenhang mit $ignore_user legt dieser Wert fest, ob alle vorhandenen Einzelrechte
	 *									des Elements zurï¿½ckgeliefert werden oder nur die Rechte des anfragenden Users
	 *									Optional - Standard: false - wenn, dann nur Rechte des anfragenden Users zurï¿½ckliefern
	 * @param	string	@parent			ID des ï¿½bergeordneten Elementes, z.B. Kategorie einer Seite, Ordner einer Datei
	 *									Optional - Standard: '0' - ï¿½bergeordnete Rechte sind Area-Rechte
	 * @param	string 	$prefix 		Prefix fï¿½r Formularvariablen, damit mehrere Formulare auf einer Seite dargestellt werden kï¿½nnen
	 * 
	...
	**/
	function get_right_panel($type, $id, $config = '', $view = 'text', $ignore_user = false, $all_users = false, $parent = '0', $prefix = '') {
		...
		if ($this->_group_count > 0) {
			$parts = array();
			$skin_path = $cfg_cms['path_base_http'] . $cfg_cms['path_backend_rel'] . 'tpl/' . $cfg_cms['skin'];
			// Erstelle Hidden_Fields - beinhalten die Rechte, die zum Element gehï¿½ren
			...
			$parts["rights"] = $out;
			// Erstelle notwendige JavaScript-Einbindungen und schreibe das Konfigurationsscript fï¿½r die Popup-Ebene
	...
	/**
	 * Setzt Gruppenrechte fï¿½r einen Rechtetyp und eine Element
	 *
	 * Rechte werden nur gesetzt, wenn sich zu den ï¿½bergeordnetne Rechten Abweichungen ergeben. Bei Abweichungen wird der
	 * komplette Rechtesatz des Elementes geschrieben, eine Vererbung einzelner Rechte ist nicht vorgesehen.
	 * Fï¿½r Vererbung werden die Rechte der Rechtehierarchie des Vater-Element geprï¿½ft.
	 *
	 * @param	string	$type 			Rechtetype der bearbeitet wird
	 * @param	string	$id				ID des Elements, dessen Rechteeintrï¿½ge verï¿½ndert werden sollen
	 ...
	 * @param	int		$bitmask		Bitmaske fï¿½r die Festlegung welche Rechte bei der ï¿½berprï¿½fung der Vererbung berï¿½cksichtigt
	 *									werden
	 *									Optional - Standard: 0xFFFFFFFF - alle ï¿½bergeordneten Rechte berï¿½cksichtigen
	 * @param	string	$parent_id		ID des ï¿½bergeordneten Elements
	 *									Optional - Standard: '0'
	 * @param	int		$second_bitmask	Bitmaske, welche eingesetzt wird, wenn die Hierachie der Vererbung ï¿½ber eine 2 Ebenen geht.
	...
	**/
	function set_group_rights( $type, $id, $gruppenids, $gruppenrights, $gruppengeerbt, $gruppenueberschreiben, $idlang = '', $bitmask = 0xFFFFFFFF, $parent_id = '0', $second_bitmask = 0xFFFFFFFF) {
		...
		// Hole jede Gruppe und jedes dazugehï¿½rende Recht und prï¿½fe ob es gegenï¿½ber dem ï¿½bergeordneten Element eine Differenz
		...
		for($i = 0; $i < count($arrGroups); $i++) {
			...
			// Addon Objekt ausfï¿½hren, falls recursive gelï¿½scht werden soll
			if ($delete_rec) {
			...
			// Lï¿½sche zunï¿½chst ggf. vorhandene alte Wert ... so dass das zurï¿½cksetzen auf den aktuellen Area-Wert funktioniert
	...
	/**
	 * Setzt Eigentï¿½merrechte fï¿½r einen Rechtetyp und eine Menge von Elementen
	 *
	 * Eigentï¿½merrechte werden durch einen negativen in der Spalte idgroup gekennzeichnet.
	 * Die Eigentï¿½merrechte sind fï¿½r spï¿½tere Erweiterungen ausgelegt und in Sefrengo Version 1.0 und vorher
	 * werden diese Rechte nicht gesetzt oder ausgelesen.
	 *
	 * @param	string	$type 			Rechtetype der bearbeitet wird
	 * @param	mixed	$idarray		ID des Elements (als int oder string) oder IDs der Elemente die Eigentï¿½mer-Rechte
	 *									bekommen
	 * @param	string	$perm_value		Bitmaske der Eigentï¿½mer-Rechte
	 * @param 	string	$userid			ID des User, der Eigentï¿½mer ist
	...
	**/
	function set_owner_rights( $type, $idarray, $perm_value, $userid = '', $idlang = '') {
		// Auskommentiert bis eigentï¿½mer-rechte ï¿½berall implementiert und bearbeitbar sind
		return true;
		...
		if (is_string($idarray) || is_int($idarray)) {
			// Einzelnes Objekt
			// GGf. bestehende Rechte des User lï¿½schen und danach neuen Perm-Eintrag erzeugen
	...
	/**
	 * Zentrale Prï¿½froutine fï¿½r die Rechte eines Users an einem Objekt oder Bereich
	 *
	 * Liefert als Ergebnis zurï¿½ck, ob ein User ein nachgefragtes Recht hat oder nicht. Hierbei kï¿½nnen verschiedene Abfragen
	 * durchgefï¿½hrt werden, die sich anhand der Parameterï¿½bergaben unterscheiden:
	 ...
	 * 2. Rechte auf das Element sind fï¿½r diesen User vorhanden
	 *		$perm_or_area	string	'0'
	 *		$type			string	Name des Rechtetyps - alle ausser 'cms_access' sind mï¿½glich
	 *		$id				string	ID des Objektes
	 *		$parent_id wird ignoriert
	 * 3. Einzelnes Recht auf das Objekt fï¿½r den User wird geprï¿½ft
	 *		$perm_or_area	int		Nummer des Rechtes
	 *		$type			string	Name des Rechtetyps - alle ausser 'cms_access' sind mï¿½glich
	 *		$id				string	ID des Objektes
	 *		$parent_id 		int		ID des ï¿½bergeordneten Objektes
	 *
	 *
	 *
	 ...
	 * @param	int		$parent_id		ID des ï¿½bergeordneten Objektes
	 *
	...
	function have_perm($perm_or_area, $type='cms_access', $id = '0', $parent_id = 0) {
		...
		//
		// Lade Perms des Types fï¿½r die Gruppen des Users und der aktuellen Sprache
		...
		if ($type == 'cms_access') {
			// Prï¿½fungstyp 1:
			// Access-Rechte prï¿½fen wird nicht auf die Bitmask bezogen, es wird lediglich geprï¿½ft,
			...
			// In einer nï¿½chsten Version sollte es mï¿½glich sein, fï¿½r Areas entsprechende Alias-Namen
			...
			// Prï¿½fungstyp 2:
			// Prï¿½ft ob fï¿½r den User ï¿½berhaupt Rechte fï¿½r ein Objekt zugeteilt sind.
			if ($perm_or_area == '0') {
				return ($this->perms[(string)$type][(string)$id] != '0');
			}
			// Prï¿½fungstyp 3: Perm-Prï¿½fung
			// $perm_or_area wird als Nummer des zutestenden Bit interpretiert
			// $type sagt welcher Bereich
			// $id ist die ID des Objekts das geprï¿½ft wird und nach Integer gecastet!
			// Der Perm-Wert fï¿½r $type, $id wird auf das gewï¿½nschte Bit getestet, liefert
			// der Test einen Wert ungleich 0, ist das Bit gesetzt und somit das Recht erteilt.
			$_mask <<= (--$perm_or_area);
			// Prï¿½fe ob es das Recht schon am Objekt selbst gibt, wenn nicht prï¿½fe ob die ï¿½bergeordneten Rechte entsprechend
			// fï¿½r den User gesetzt sind.
			
			// Prï¿½fe ersten Block
			// Rechte kï¿½nnen sein: slave, master oder area
			$original_id = $id;
			do {
				if (isset($this->perms[(string)$type][(string)$id])) {
					// Es existiert ein Eintrag fï¿½r das Element
					if ($original_id != $id) {
						// Cache value
			...
			// Es existiert kein Eintrag fï¿½r das Element, teste ï¿½bergeordnete Perm-Eintrï¿½ge.
			// Hole ï¿½bergeordnete Rechtetypen aus der Parent-Liste
			if ($parent_id >= 0) {
				$child_type = $type;
				...
				// Prï¿½fe zweiten Block
				// Rechte kï¿½nnen sein: master oder area
				if (isset($this->parent_rights[$child_type])) {
					
					// ï¿½bergeordnete rechtegruppe ermitteln
					...
					// Prï¿½fe dritten Block - es kann sich nur noch um ein area recht handeln
	...
	function new_perm( $group, $type, $id = 0, $permission = 0, $lang = 0) {
		...
		if (abs($group) == 1) return true;	// keine Eintrï¿½ge fï¿½r User ohne Gruppenzugehï¿½rigkeit
		// Ermittle zu setzenden Wert fï¿½r perm aus der ï¿½bergabe $permission
		//echo '<br /><br /><br /><br />';
		//echo $permission .'<br />';
	 ...
	 * @param mixed		$permission		Werte fï¿½r perm
	 *									Ist $permission ein Array aus Integern, so wird die Summe der Array-Werte
	 *									eingetragen. Ist $permission ein Integer, wird der Wert eingetragen
	 ...
	 */
	 function update_perm_by_id($idperm, $group, $type, $id, $permission, $lang) {
		// Prï¿½fe ob idperm, $group und $type gesetzt sind
	 ...
	 * Lï¿½scht Userperms
	 *
	 * Abhï¿½ngig von der Parameterkombination sind folgende Lï¿½schungen mï¿½glich
	 *
	 * $idperm gesetzt:
	 * 1.	$idperm	> 0      :	Lï¿½schen des angegebenen Rechtes
	 * 2.   $idperm ist Array:  Lï¿½schen der angegebenen Rechte
	 *
	 * $idperm nicht gesetzt, dafï¿½r $type und $id gesetzt
	 * 3.	$id, $type		 :	Gesetzte Einzelwerte -> Lï¿½scht alle Rechte die $id und $type umfassen
	 *
	 *		3a. $ignore_lang :	true  -> lï¿½sche alle Eintrï¿½ge unabhï¿½ngig von der Sprache
	 *							false -> berï¿½cksichtige die Sprache beim Lï¿½schen
	 *							a. $lang Einzelwert:	Nur die Eintrï¿½ge einer Sprache lï¿½schen
	 *							b. $lang Array:			Alle Eintrï¿½ge der Sprachen lï¿½schen
	 *
	 *		3b. $group		 :  gesetzt: berï¿½cksichtige die Gruppenangabe beim Lï¿½schen
	 *							a. $group Einzelwert:	Nur die Eintrï¿½ge einer Gruppe lï¿½schen
	 *							b. $group Array:		Alle Eintrï¿½ge der Gruppen lï¿½schen
	 *
	 * 4. Berï¿½cksichtige entsprechend 3a. und 3b. Sprache und Usergruppen beim Lï¿½schen
	 *
	 *
	 *
	 * @param	int		$id		ID des Objektes, optional - Standard: 0, kein Objektrecht
	 ...
	 *							Ist $idperm ein Array aus Integern, so werden alle angegebenen Rechte gelï¿½scht.
	 *							Ist $idperm ein Integer, wird das angegebene Recht gelï¿½scht.
	 * @param	int		$lang	Sprache-ID, optional - Standard: 0
	 * @param	boolean	$ignore_lang	Sprache beim Lï¿½schen nicht berï¿½cksichtigen (true), optional - Standard: false
	 *
	 * @return	bool 	true, wenn Rechte gelï¿½scht wurden, ansonsten false.
	 *
	 * @author	Jï¿½rgen Brï¿½ndle
	 * @since	CMS 0.96.00 / Beta
	 * @version 0.1
	...
	/**
	 * Lï¿½scht Perm-Eintrï¿½ge einer oder mehrerer Gruppen
	 *
	 * Abhï¿½ngig von der Parameterkombination sind folgende Lï¿½schungen mï¿½glich
	 *
	 * $group gesetzt:
	 * 1.	$group	> 0     :  Lï¿½schen der Rechte der Gruppe
	 * 2.   $group ist Array:  Lï¿½schen der Rechte der Gruppen
	 * ist $idlang gesetzt, wird die Sprache beim Lï¿½schen berï¿½cksichtigt:
	 * 1.	$lang	> 0     :  Lï¿½schen der Rechte der Sprache
	 * 2.   $lang ist Array :  Lï¿½schen der Rechte der Sprachen
	 *
	 *
	 * @return	bool 	true, wenn Rechte gelï¿½scht wurden, ansonsten false.
	 *
	 ...
	 * Drei verschiedene Test sind mï¿½glich:
	 * $test_type
	 * 		0 		existenz prï¿½fen unter ausschluss des wertes 0
	 * 				($this->perms[$type][$id] && $perm->perms[$type][$id] != '0')
	 * 		8 		existenz prï¿½fen unter ausschluss des wertes 0
	 * 				($this->perms[$type][$id] && $perm->perms[$type][$id] != '0')
	 * 		9 		existenz prï¿½fen unter ausschluss des wertes 0
	 * 				($this->perms[$type][$id] && $perm->perms[$type][$id] != '0')
	 *
	 * WICHTIG: test_perm kann nicht verwendet werden, wenn geprï¿½ft werden soll, ob ein User in einer
	 ...
	 * Werden bestimmte Gruppenrechte benï¿½tigt sollte die Funktion get_userperm() oder find_perm() genutzt werden.
	 *
	 * Gibt bei unbekanntem Test oder bei Fehlern false zurï¿½ck.
	 *
	 * @param int		$type			Rechtegruppe
	 * @param int		$id				Objekt-ID
	 * @param int		$test_type		durchzufï¿½hrender Test, optional - Standard: 0
	 ...
	 */
	 function test_perm($type, $id, $test_type = 0, $value = 0, $ignore_admin = false) {
		...
		$this->_retrieve_perms((string)$type);
		// prï¿½fe ob idperm gesetzt oder $group und $type gesetzt sind
		switch ((int)$test_type) {
			case 0: // existenz prï¿½fen unter ausschluss des wertes 0
				if ($this->perms[$type][$id] && $this->perms[$type][$id] != '0') return true;
				return ((int)$this->perms[$this->parent_rights[$type]]['0'] > 0);
			case 8: // Werteprï¿½fung
				return (!empty($this->perms[$type][$id]) && $this->perms[$type][$id] == $value);
			case 9: // Bitmask-Prï¿½fung 1 - prï¿½ft ob bestimmte Bitmask gesetzt ist
	...
	/**
	 * Testet Userperms auf bestimmte Eigenschaften und liefert " checked " oder einen leeren String zurï¿½ck
	 * Benutzt test_perm fï¿½r die Tests, Parameter und Testtypen siehe dort.
	 *
	 * @param int		$type			Rechtegruppe
	 * @param int		$id				Objekt-ID
	 * @param int		$test_type		durchzufï¿½hrender Test, optional - Standard: 0
	...
	/**
	 * Ermittelt die Rechte einer betimmten Rechtegruppe fï¿½r ein Objekt fï¿½r eine oder mehrere Usergruppen
	 * Benutzt get_perm, Details zur Ermittlung der Rechte siehe dort.
	 *
	 * Fï¿½r die Ermittlung der Rechte wird die aktuelle Anzeigesprache verwendet
	 *
	 * @param	string	$type	Rechtegruppe
	...
	/**
	 * Ermittelt die Besitzerrechte fï¿½r ein Element in der gewï¿½nschten Sprache
	...
	*/
	...
	/**
	 * Ermittelt die Perm-Eintrï¿½ge eines Objekts
	 ...
	 * @param	bool	$apply_owner_rights	Besitzerrechte berï¿½cksichtigen, optional - Standard: true - Besitzerrechte
	 *										berï¿½cksichtigen
	 *
	...
	/**
	 * Ermittelt alle Perms eines Objekts in einer bestimmten Rechtegruppe fï¿½r eine oder mehrere Usergruppen
	 * als Summe aller Rechte dieses Objekts
	 *
	 * Wenn als Gruppenangabe eine Liste von Gruppen ï¿½bergeben wird, dann werden alle Rechte der einzelnen Gruppen als
	 * Kombinationswert aller einzelnen Rechte zurï¿½ckgeliefert, andernfalls werden nur die Rechte einer einzelnen Gruppe
	 * als Ergebnis zurï¿½ckgeliefert.
	 * Negative Gruppen-IDs kennzeichnen Eigentï¿½merrechte, diese ï¿½berstimmen die allgemeinen Rechte in der Form, das die
	 * Kombination der einzelnen Rechte zunï¿½chst um die Besitzerrechte erweitert werden und dann anschlieï¿½end auf diese Rechte
	 * eingeschrï¿½nkt zu werden.
	 *
	 * @param	int		$perm_return		Perm-Wert der ermittelt wurde als Rï¿½ckgabe ï¿½ber eine Referenz-Variable
	 * @param	mixed	$group				Array oder Integer mit Gruppen-IDs
	 * @param	string	$type				Rechtegruppe
	 * @param	int		$id					Objekt-ID, optional - Standard: 0 - Area-Rechte
	 * @param	int		$lang				Sprach-ID, optional - Standard: 0 - keine sprachspezifischen Rechte
	 * @param	bool	$apply_owner_rights	Besitzerrechte mit berï¿½cksichtigen, optional - Standard: true - Besitzerrechte
	 *										berï¿½cksichtigen
 	 *
	 * @return	bool	true, wenn Perm-Eintrï¿½ge gefunden wurden, sonst false
	 *
	 * @author	Jï¿½rgen Brï¿½ndle
	 * @since	CMS 0.96.00 / BETA
	 * @version 0.1
	**/
	...
	/**
	 * Gibt einen array mit allen Ids der Gruppen zurï¿½ck, in der der der aktuelle Benutzer Mitglied ist
     *
	 * @return	array	alle Gruppenids in der der Benutzer Mitglied ist
	 *
	 * @author	Bjï¿½rn Brockmann
	...
	/**
	 * Kopiert Perm-Eintrï¿½ge innerhalb einer Rechtegruppen
	 * Benutzt xcopy_perm mit gleicher Rechtegruppe fï¿½r Quelle und Ziel-Objekt, Details siehe dort.
	 *
	 * @param	int		$idorigin		Objekt-ID des Quell-Objekts
	 * @param	string	$type			Rechtegruppe
	 * @param	int		$idtarget		Objekt-ID des Ziel-Objekts
	 * @param	int		$group			Usergruppe deren Perm-Eintrï¿½ge kopiert werden sollen, optional - Standard: 0-alle Gruppen
	 * @param	int		$lang			Sprache die beim Kopieren berï¿½cksichtigt werden soll, optional - Standard: 0-keine Sprache
	 * @param	bool	$ignore_lang	true, wenn Perm-Eintrï¿½ge aller Sprachen kopiert werden sollen, ansonsten false.
	 *									optional - Standard: true - perm-Eintrï¿½ge aller Sprachen kopieren
	 *
	...
	/**
	 * Kopiert Perm-Eintrï¿½ge zwischen verschiedenen Rechtegruppen
	 ...
	 *
	 * Change: 02052004 - jb - Aufrufparameter $id fï¿½r _copy_perm-Aufruf hinzugefï¿½gt, Workflow und SQL optimiert
	 */
	 function xcopy_perm($idorigin, $typeorigin, $idtarget, $typetarget, $keepbitmask = 0xFFFFFFFF, $group = 0, $lang = 0, $ignore_lang = true) {
		// prï¿½fe ob notwendigen angaben gesetzt sind
		...
		// Erstelle SQL-Statement zum Auslesen der Perm-Eintrï¿½ge
	...
	/**
	 * Kopiert Perm-Eintrï¿½ge eines Objektes
	 *
	 * Als ï¿½bergabe wird ein SQL verwendet, der die zu kopierenden Rechte ausliest. Als weiterer Parameter wird die ID des
	 * Zielobjektes ï¿½bergeben, die beim Kopieren der Recht an Stelle der ursprï¿½nglichen Objekt-ID eingetragen wird.
	 *
	 * @param	string	$sql		SQL-Statement zum Auslesen von Perm-Eintrï¿½gen
	 * @param	int		$targetid	Objekt-ID des Objekts, das die Perm-Eintrï¿½ge erhalten soll
	 *
	 * @author	Jï¿½rgen Brï¿½ndle
	 * @since	CMS 0.96.00 / BETA
	 * @version 0.2 / 20040502
	 *
	 * Change: 02052004 - jb - Aufrufparameter $targetid hinzugefï¿½gt
	 */
	...
	/**
	 * Ermittelt ob es fï¿½r ein Objekt Rechteeintrï¿½ge gibt oder nicht.
	 * Unterscheidet anhand von $ignore_user, ob Benutzerrechte mit berï¿½cksichtigt werden oder nicht.
	 *
	 * Liefert als Ergebnis die Anzahl der Usergruppen zurï¿½ck, fï¿½r die Rechteeintrï¿½ge bestehen.
	 *
	 * @param	int		$id				Objekt-ID
	 * @param	string	$type			Rechtegruppe
	 ...
	 */
	 function perms_existing($id, $type, $ignore_user = true) {
		...
		// Ermittelte Anzahl zurï¿½ckgeben
		return $count;
	 }

	/**
	 * Ermittelt die Anzahl von Objekten fï¿½r ein bestimmtes Recht und seiner ï¿½bergeordneten Rechteeintrï¿½ge
	 *
	 * Liefert als Ergebnis die Anzahl der Objekte zurï¿½ck, fï¿½r die Rechteeintrï¿½ge bestehen.
	 *
	 * $group gesetzt:
	 * 1.	$group	> 0     :  Prï¿½fen der Rechte in der Gruppe
	 * 2.   $group ist Array:  Prï¿½fen der Rechte in den Gruppen
	 * ist $idlang gesetzt, wird die Sprache beim Lï¿½schen berï¿½cksichtigt:
	 * 1.	$lang	> 0     :  Prï¿½fen der Rechte in der Sprache
	 * 2.   $lang ist Array :  Prï¿½fen der Rechte in den Sprachen
	 *
	 *
	 * @param	mixed	$type		Rechtegruppe (Parents werden nicht ermittelt!)
	 *								Rechtegruppen als Array
	 * @param	int		$perm		Zu prï¿½fendes Recht oder zusammenfassung mehrerer Rechte
	 * @param	mixed	$group		Prï¿½fen des Rechts der Gruppe (int) oder den Gruppen (array)
	 *                              Ist kein Wert gesetzt, werden die aktuellen Gruppenparameter des perm- Objekts genommen
	 * @param	mixed	$lang		Prï¿½fen des Rechts der Sprache (int) oder den Sprachen (array)
	 *
	...
	/**
	 * Gibt die ID des aktuellen Projektes zurï¿½ck
	 *
	...
	**/
	...
	/**
	 * Gibt die aktuelle Spracheeinstellung zurï¿½ck
	 *
	...
	**/
	...
	/**
	 * Gibt die Zeichensatzcodierung fï¿½r der aktuellen Spracheeinstellung zurï¿½ck
	 *
	...
	**/
	 ...
	 * Check-Methoden
	 *
	 * Prï¿½fung bestimmter Rechte, die fï¿½r die weitere Verarbeitung unabdingbar sind. Sind entsprechende Rechte nicht zugeteilt,
	 * wird die Datei 'perminvaild.php' eingebunden und das Script beendet.
	**/

	/**
	 * Prï¿½ft ob User in der Gruppe der Systemadministratoren ist
	 ...
	 * Hat der User nicht das geforderte Rechte wird die Datei 'perminvalid.php' aus dem aktuellen Skin eingebunden und die
	 * Scriptverarbeitung beendet.
	 * Nutzt zur Rechteprï¿½fung die methode have_perm, details siehe dort.
	 *
	 * @param	string		$perm_to_check	Zu prï¿½fendes Recht
	 * @param	string		$perm_cat		Rechtegruppe, optional - Standard: 'cms_access' - Zugriffsrechte
	 * @param	string		$id				Objekt-ID, optional - Standard: 0 - kein Objekt-Recht zur Prï¿½fung
	...
	function check($perm_to_check, $perm_cat='cms_access', $id='0', $parent_id = 0) {
		$this->cms_log->debug('sf_core', 'Test Perms: '. $perm_to_check .', cat: '.$perm_cat .', id: '. $id, array('file' => basename(__FILE__), 'line' => __LINE__));
		if (!$this->have_perm($perm_to_check, $perm_cat, $id, $parent_id)) $this->perm_invalid();
	}

	/**
	 * Wird aufgerufen, wenn der User ein bestimmtes Recht nicht hat und dieses mit check() geprï¿½ft wird.
	...
	**/
	function perm_invalid() {
		...
		include $cfg_cms['path_base'].$cfg_cms['path_backend_rel'].'tpl/'.$cfg_cms['skin'].'/perminvalid.php';
		exit;
	}

	/**
	 * Gibt das erste Area zurï¿½ck, fï¿½r die der User das Recht hat, die Area zu betreten (Rechtegruppe cms_access)
	 *
	...
	**/
	...
	/**
	 * Ermittelt Perm-Eintrï¿½ge fï¿½r eine Rechtegruppe und speichert diese in einer internen Liste der Perm-Werte zwischen
	 *
	 * Fï¿½gt dabei nur Rechtegruppen in die interne Liste ein, die noch nicht ausgelesen wurden.
	 *
	 *
	 * @param	string	$type	Rechtegruppe
	 * @param	int		$lang	Sprach-ID
	...
	**/
	function _retrieve_perms($type = '', $lang = '', $group = '') {
		// Prï¿½ft ob der Rechteblock schon geladen ist
		if (!empty($this->loaded_types[(string)$type])) return true;
		$this->cms_log->debug('sf_core', 'Requested block "'.$type.'" will be loaded...', array('file' => basename(__FILE__), 'line' => __LINE__));

		// Prï¿½fe ob Sprache und Gruppe gesetzt sind, wenn nicht setze Default-Werte
		// $this->lang fï¿½r Sprache und $this->idgroup fï¿½r die Gruppenliste
		$r_lang  = (!empty($lang))  ? $lang : (empty($this->lang) ? '0': $this->lang);
		$r_group = (!empty($group)) ? $group: $this->idgroup;
		// Lade alle Perms der Rechtegruppe $type fï¿½r die Sprache $r_lang und die Gruppen $r_group
	...
	/**
	 * Fï¿½gt der lokalen Liste der ausgelesenen Perm-Werte einen Wert hinzu, wenn dieser noch nicht in der Liste enthalten ist.
	 *
	...
	**/
	function _find_out_lang($client, $lang, $changeclient, $lang_charset) {
		...
		if(empty($client) || empty($lang) || $changeclient == 1){
			if($changeclient =='1'){
				...
				if ($this->_is_admin) {
					//admin wechselt sprache, erste verfï¿½gbare nehmen
					...
					//normaler user, erste sprache nehmen, fï¿½r die er ein recht hat
					$sql  = "SELECT DISTINCT cl.idclient, cl.idlang ";
	...
	/**
	 * Setzt die Liste aller Gruppen fï¿½r den aktiven Benutzer
	 * Prï¿½ft gleichzeitig, ob der Benutzer zur Gruppe der Systemadministratoren gehï¿½rt.
	**/
	...
	/**
	 * Liest die Gruppeninformationen fï¿½r den zu konfigurierenden Benutzer in die entsprechenden Eigenschaften der Klasse ein
	**/
	function _get_group_infos_from_user_simulate() {
		...
		if ($this->db->next_record()) {
			...
			$this->cms_log->error('sf_core', 'No Groups found (File: '.__FILE__.')');
		}
	}

	/**
	 * Ermittelt alle Usergruppen und alle Rechte zu einem Objekt und liefert das Ergebnis in Form dreier CSV-Listen in einem
	 * Array zurï¿½ck. Die Rechte werden durch die Vererbung ggf. aus verschiedenen Ebenen der Rechtehierarchie ausgelesen.
	 *
	 * WICHTIG: Access-Rechte werden nicht ausgelesen!
	 *
	 * @param	string	$type			Rechtegruppe
	 * @param	int		$id				Objekt-ID
	 * @param	bool	$include_user	True, wenn Rï¿½ckgabe Besitzerrechte umfassen soll, sonst false.
	 * @param	bool	$all_user		True, wenn Rï¿½ckgabe alle Besitzerrechte umfassen soll, sonst false. Nur in Verbindung mit
	 *									$include_user
	 * @param	int		$parent			ID der Parent-Objekts
	 *
	 * @global	$lang
	 *
	 * @return	int		Rechte-Bitmaske aus den Werten des Array oder Integer aus der ï¿½bergabe
	 *
	 * @author	Jï¿½rgen Brï¿½ndle
	 * @since	CMS 0.96.00 / BETA
	...
	**/
	function _get_rights_by_groups_user($right_type, $id, $include_user, $all_user, $parent) {
		...
		$is_first_run = true;
		do {
			// Ermittle alle vergebene Rechte fï¿½r das Element
		...
		// Setze Area-Rechte fï¿½r alle ungesetzten Gruppenrechte
		// Schleife wegen drei Ebenen-Vererbung ...
		//
		// Erster Durchgang liest die Rechte aus, die der ï¿½bergeordneten Rechtegruppe (Ordner/kategorien) und einer ParentID
		// zugeordnet sind. Ist die ParentID nicht 0, wird die Schleife nochmals mit der ï¿½bergeordneten Rechtegruppe und dem
		// Wert '0' fï¿½r die ParentID durchlaufen, erst danach geht es auf die nï¿½chsthï¿½here Ebene (Area-Ebene), die immer mit
		// dem Wert '0' fï¿½r die ParentID arbeitet.
		//
		...
		// Liefere ein Array mit CSV fï¿½r Gruppen und Userrechte zurï¿½ck
	...
	/**
	 * Erstellt eine Bitmask fï¿½r die Rechte durch Addition von Array-Werten
	 *
	 * @param	mixed	$permission		Array mit den Integerwerten fï¿½r die Rechte oder einzelner Integerwert fï¿½r ein Recht
	 *
	 * @return	int		Rechte-Bitmaske aus den Werten des Array oder Integer aus der ï¿½bergabe
	 *
	 * @author	Jï¿½rgen Brï¿½ndle
	...
	******************************************************************/
	...
	/**
	 * Erstellt das Rechtepanel fï¿½r die Darstellung bei der Bearbeitung von Objekten
	 *
	 ...
	 *
	 * ï¿½nderung der Schnittstelle, weil in Mozilla dynamische Formulare, die aus JS erstellt werden, nicht mit JS angesprochen
	 * werden kï¿½nnen. Schnittstelle gibt jetzt den Inhalt fï¿½r das Rechtepanel zurï¿½ck anstatt einer Liste der Rechtenamen und
	 * Rechtewerte.
	 *
	...
	**/
	function _get_right_names_and_values( $type, $rights, &$countRights , $prefix) {
		...
		if ( $this->perm_addon[$type]->showDeleteChildsCheckbox($type)) {
			$rights_list .= '<tr onmouseover="this.style[\'background\']=\'#FFF5CE\';" onmouseout="this.style[\'background\']=\'#FFFFFF\';">';
			$rights_list .= '  <td class="first nowrap" align="left">' . '<i>'.$cms_lang['panel_inherit_perms'].'</i>' . '</td>';
		...
		// Erzeuge Kopf des Panels - ï¿½berschrift, Gruppen-DropDown
		$panel  = "\n".'<div id="rightsmenucoat"><div class="header">';
		$panel .= '<a href="#" class="sfCloseLink" onclick="'.$prefix.'cms_rm.cancelRights(true);return false;">&#215;</a>'."\n";
		// ï¿½berschrift
		...
		$panel .= '    </select>';
		$panel .= '</p>'."\n";
		// Checkbox Rechte des Vorgï¿½ngers benutzen
		$panel .= '<p>'."\n";
		$panel .= '    <input type="checkbox" name="'.$prefix.'rmerben" id="'.$prefix.'rmerben" value="1" onclick="'.$prefix.'cms_rm.handleRadioReadonly()" /> <label for="'.$prefix.'rmerben">'.$cms_lang['panel_inherit_perms'].'</label>'."\n";
		$panel .= '</p>'."\n";
		...
		// Zeile fï¿½r alle Rechte, zunï¿½chst noch gesperrt ...
...
//		$panel .= '</tr-->';
//		// Zeile fï¿½r Rechte auf Unterelemente vererben, zunï¿½chst noch gesperrt ...
//		$panel .= '<!--tr onmouseover="this.style[\'background\']=\'#C7D5EB\';" onmouseout="this.style[\'background\']=\'#DBE3EF\';">';
//		$panel .= '  <td class="rightsname nowrap" align="left">' . $cms_lang['panel_all_rights'] . '</td>';
//		$panel .= '  <td class="rightscell" align="center">';
//		$panel .= '<a href="javascript:void(0)" onclick="'.$prefix.'cms_rm.setAllRights(-1);">' . $cms_lang['panel_all_rights_denied'] . '</a></td>';
...
//		$panel .= '</tr-->';
	...
	/**
	 * Erstellt eine Dropdown-Liste der ï¿½bergebenen Gruppen fï¿½r das Rechtepanel
	 *
	...
	**/
	...
	/**
	 *
	 * Erstellt eine Bedingung fï¿½r eine SQL-Qeury, die einen bestimmten Wert sucht oder einen bestimmten Wert innerhalb einer
	 ...
	 *										optional
	 *
	 * @return	string	Bedingung fï¿½r SQL-Query als IN- oder =-Bedingungsfragment fï¿½r den Zusammenbau der SQL-Query
	 *
	 * @author	Jï¿½rgen Brï¿½ndle
	 * @since	CMS 0.96.00 / Beta
	 * @version 0.1
	**/
	 ...
	 *
	 * Berï¿½cksichtigt die Usergruppen "keine" und "Administrator" fï¿½r die Zï¿½hlung
