<?php 
// File: $Id: fnc.type_forms.php 684 2012-04-10 00:57:15Z bjoern $
// +----------------------------------------------------------------------+
...
{
	if (! _type_check_editable($type_config['editable'])) return _type_get_element_hidden($formname, $content);
    else {
...
* @Access public 
*/
function type_form_textarea($formname, $content, $type_config)
{
    global $cfg_cms, $sess;
    ...
    if ($type_config['transform'] == 'bbcode') {
        ...
        <input style="color: rgb(0, 0, 255); text-decoration: underline;" name="mail" value="E-Mail" title="E-Mail mit Text einf�gen: [email=name@adresse.de]email an [/email]" onclick="insert_mail();" type="button">
        <input style="color: rgb(0, 0, 255); text-decoration: underline;" name="link2" value="Link" title="Link mit Linktext einf�gen: [link=http://...]Linktext[/link]" onclick="insert_link();" type="button"><br>
        <textarea id="text" name="' . $formname . '" rows="14" cols="52" style="width:' . $width . ';height:' . $height . '">' . $content . '</textarea>
        <script type="text/javascript" src="' . $sess->url($cfg_cms['path_base_http'] . $cfg_cms['path_backend_rel'] . 'tpl/standard/js/bbcode.js') . '"></script>
        </td>' . "\n";
    } else {
        // ohne bbcode
        return '<td>
...
{
    global $cms_lang, $client, $lang, $dedi, $cfg_cms, $cfg_client, $sess;

    // echo "<pre>".$content."</pre><br><hr>";
    // make relative pathes to absolut pathes
    $upl_path_relative = $cfg_client['path_rel'] . $cfg_client['path_fm_rel'];
    // $preg = '!(src|href)([ ]*=[ ]*("|\'|&quot;))('.$upl_path_relative.'[\w/.]*("|\'|&quot;))!';
    // $preg_out = '\\1\\2'.$cfg_client['htmlpath'].'\\4';
    // $content = preg_replace($preg, $preg_out, $content);
    // echo "<pre>".$content."</pre><br><hr>";
    $content = str_replace('&quot;' . $upl_path_relative, '&quot;' . $cfg_client['path_http'] . $cfg_client['path_rel'] . $upl_path_relative, $content);
    $out = '<script src="' . $sess->url($cfg_client['path_http'] . $cfg_client['path_rel'] . 'cms/ckeditor/ckeditor.js') . '"></script>' . "\n";
    // Textfeld erstellen
    ...
    $type_config['sf_idlay'] = $cfg->env('idlay');
    unset($cfg);

    $toolbar_set = (trim($type_config['features']) == 'true' || trim($type_config['features']) == '')
    ? 'SefrengoDefault' : $formname;
    
	$out .= "var sf_BasePath = '" . $cfg_client['path_http'] . $cfg_client['path_rel'] . "cms/ckeditor/';
	CKEDITOR.replace(
		'" . $formname . "',
		{
			toolbar : '" . $toolbar_set . "',
...
* @Access public 
*/
function type_form_img($formname, $content, $type_config)
{
    ...
    $rb->setJSCallbackFunction('sf_getImage', array('"'.$formname.'"', 'picked_name', 'picked_value'));
    $rb_url = $rb->exportConfigURL();

    if (is_file($sf_cfg->env('path_frontend_fm') . $thumb_url)) {
        $img_src = $sf_cfg->client('path_fm_rel') . $thumb_url;
        $img_thumb_x = $pic_thumb_width;
        $img_thumb_y = $pic_thumb_height;
    } else if ($pic_height > 0 && $pic_width > 0) {
        $img_src = $sf_cfg->client('path_fm_rel') . $pathway_string;
...
<img id="' . $formname . '" src="' . $img_src . '"  border="0" width="' . $img_thumb_x . '" height="' . $img_thumb_y . '" />
    ...
    $out .= "<input type=\"hidden\" id=\"" . $formname . "_value\" name=\"$formname\" value=\"$content\">
    <input type=\"text\" id=\"" . $formname . "_display\" name=\"" . $formname . "display\" readonly=\"readonly\" value=\"" . $pathway_string . "\" style=\"width:560px\" >\n";
    $out .= "<input type='button' value='DEL' onclick=\"sf_getImage('" . $formname . "', '', '');\" />";
    $out .= "&nbsp;<input type='button' value='&hellip;' onclick=\"SF.Utils.popup('$rb_url', 'rb', '', screen.width * 0.7, screen.height * 0.7, 'true')\" />";
    $out .= '</td></tr></table></td>' . "\n";
    $out .= '<script type="text/javascript">
	<!--
	function sf_getImage(input_id, name, value) {
		document.getElementById(input_id + "_value").value= value;
		document.getElementById(input_id + "_display").value= name;
		SF.Utils.loadPreviewPic("' . $sf_cfg->env('path_frontend_fm_http') . '"+name, "' . $sf_cfg->client('thumbext') . '", input_id);
	}
	-->
	</script>';
    return $out;
...
* @Access public 
*/
function type_form_imgdescr($formname, $content, $type_config)
{
...
* @Access public 
*/
function type_form_link($formname, $content, $type_config)
{
    global $db, $cms_db, $cms_lang, $cms_lang, $client, $lang, $catlist;
    ...
    $out .= "        <tr>\n";
    $out .= "          <td ><input type=\"text\" name=\"" . $formname . "extern\" value=\"" . $sf_link_extern . "\" style=\"width:800px\" onchange=\"editcontent." . $formname . ".value=this.value; editcontent." . $formname . "intern.value='';\">&nbsp;<small>(" . $cms_lang['link_extern'] . ")</small>&nbsp;</td>\n";
    $out .= "        </tr><tr>";
    $out .= "          <td><input type=\"text\" name=\"" . $formname . "intern\" readonly=\"readonly\" value=\"" . $sf_link_intern . "\" style=\"width:640px\" >\n";
    $out .= "          <input type='button' value='DEL' onclick=\"sf_getLink" . $formname . "('', '')\" />&nbsp;<input type='button' value='...' onclick=\"SF.Utils.popup('$rb_url', 'rb', '', screen.width * 0.7, screen.height * 0.7, 'true')\" /><br />&nbsp;<small>(" . $cms_lang['link_intern'] . ")</small>
...
* @Access public 
*/
function type_form_linkdescr($formname, $content, $type_config)
{
...
* @Access public 
*/
function type_form_linktarget($formname, $content, $type_config)
{
    global $cms_lang;
    ...
    if ($content == '_self') $out .= "            <option value=\"_self\" selected=\"selected\">&nbsp;" . $cms_lang['link_self'] . "</option>\n";
    else $out .= "            <option value=\"_self\">&nbsp;" . $cms_lang['link_self'] . "</option>\n";
    if ($content == '_blank') $out .= "            <option value=\"_blank\" selected=\"selected\">&nbsp;" . $cms_lang['link_blank'] . "</option>\n";
    else $out .= "            <option value=\"_blank\">&nbsp;" . $cms_lang['link_blank'] . "</option>\n";
    if ($content == '_parent') $out .= "            <option value=\"_parent\" selected=\"selected\">&nbsp;" . $cms_lang['link_parent'] . "</option>\n";
    else $out .= "            <option value=\"_parent\">&nbsp;" . $cms_lang['link_parent'] . "</option>\n";
    if ($content == '_top') $out .= "            <option value=\"_top\" selected=\"selected\">&nbsp;" . $cms_lang['link_top'] . "</option>\n";
    else $out .= "            <option value=\"_top\">&nbsp;" . $cms_lang['link_top'] . "</option>\n";
    $out .= "          </select></td>\n";
    $out .= "        </tr>\n";
    $out .= "      </table></td>\n";
    return $out;
...
* @Access public 
*/
function type_form_sourcecode($formname, $content, $type_config)
{
	$options = array(
		//'value' => '',
    ...
    $out = "    <td>\n";
	$out .= '<textarea '.$code_attributes.' class="codemirror w800" name="'.$formname.'" id="'.$formname.'" rows="26" cols="52" wrap="off">'.$content.'</textarea>';
    $out .= "    </td>\n";
    return $out;
} 
...
* @Access public 
*/
function type_form_file($formname, $content, $type_config)
{
    ...
    // zus�tzliche beschreibung nur wenn ein title im tag angegeben wurde, sonst w�rde hier zwei mal das gleiche stehen
    ...
    $html_out .= "<tr><td>
					<input type=\"hidden\" id=\"" . $formname . "_value\" name=\"$formname\" value=\"$content\">
					<input type=\"text\" id=\"" . $formname . "_display\" name=\"" . $formname . "display\" readonly=\"readonly\" value=\"" . $pathway_string . "\" style=\"width:660px\" >\n";
    $html_out .= "<input type='button' value='DEL' onclick=\"sf_getFile" . $formname . "('', '')\" />&nbsp;<input type='button' value='...' onclick=\"SF.Utils.popup('$rb_url', 'rb', '', screen.width * 0.7, screen.height * 0.7, 'true')\" />";

    $html_out .= '	   </td>
	  </tr>
	 </table>
	</td>'; 
    // Hidetarget auslesen und variable ins filetarget r�berretten, da die Werte der cms:tags nur im Haupttag sichtbar sind
...
* @Access public 
*/
function type_form_filedescr($formname, $content, $type_config)
{
    return '<td><input type="text" name="' . $formname . '" style="width:800px" value="' . $content . '"></td>' . "\n";
} 
...
* @Access public 
*/
function type_form_filetarget($formname, $content, $type_config)
{
    global $cms_lang, $filetarget_is_hidden;
...
* @Access public 
*/
function type_form_select($formname, $content, $type_config)
{
    global $cms_lang;
...
* @Access public 
*/
function type_form_hidden($formname, $content, $type_config)
{ 
...
* @Access public 
*/
function type_form_checkbox($formname, $content, $type_config)
{
    global $cms_lang;
...
* @Access public 
*/
function type_form_checkboxsave($formname, $content, $type_config)
{ 
    // return _type_get_element_hidden($formname, $content);
...
* @Access public 
*/
function type_form_radio($formname, $content, $type_config)
{
    global $cms_lang;
