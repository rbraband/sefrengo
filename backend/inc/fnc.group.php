<?PHP
// File: $Id: fnc.group.php 619 2012-01-27 21:27:34Z holger $
// +----------------------------------------------------------------------+
...
function group_save_perms() {
	global $perm, $cms_perm_val, $db, $cms_db, $idgroup, $idlang;//, $val_ct;
	...
	if (is_array($overwrite_existing_perms)) {
		...
		$valcol->getByGroup('user_perms');
		$perm_meta = $valcol->getAssocKeyArray();
		unset($valcol);
		//print_r($perm_meta);
...
function getTitleByIdgroup($idgroup) {
	global $cms_db, $db;
