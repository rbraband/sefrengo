<?PHP
// File: $Id: inc.clients.php 641 2012-02-18 02:33:32Z bjoern $
// +----------------------------------------------------------------------+
...
switch($action) 
{
	case 'save_new_client':  //neues Projekt
		...
		//Log
		if($errlog == 0) {
			$cms_log->info('user', 'clients_new_client', array('idclient' => $cid, 'name' => $project_name));
		}
		// Event
		fire_event('clients_new_client', array(
			'idclient' => $cid,
			'name' => $project_name,
			'description' => $newdesc,
			'path' => $newpath,
	...
	case 'save_edited_client':  //Projekt umbenennen
		...
		//Log
		if($errlog == 0) {
			$cms_log->info('user', 'clients_rename_client', array('idclient' => $cid, 'name' => $newname));
		}
		// Event
		fire_event('clients_rename_client', array(
			'idclient' => $cid,
			'name' => $newname,
			'description' => $newdesc,
			'author' => $auth->auth['uid'],
	...
	case 'delete_client':  //Projekt löschen
		...
		$clientname = getClientNameByIdclient($cid);
		$errno = clients_delete_client($cid);
		$errlog  = ($errno) ? $errno : 0;
		//Log
		if($errlog == 0) {
			$cms_log->info('user', 'clients_delete_client', array('idclient' => $cid, 'name' => $clientname));
		}
		// Event
		fire_event('clients_delete_client', array(
			'idclient' => $cid,
			'name' => $clientname,
			'author' => $auth->auth['uid'],
			'errno' => $errlog
	...
	case 'save_new_lang':  // neue Sprache anlegen
		...
		//Log
		if($errlog == 0) {
			$cms_log->info('user', 'clients_new_lang', array('idclient' => $collapse, 'clientname' => $clientname, 'idlang' => $idlang, 'name' => $newname));
		}
		// Event
		fire_event('clients_new_lang', array(
			'idclient' => $collapse,
	...
	case 'save_edited_lang':  // Sprache umbenennen
		...
		//Log
		if($errlog == 0) {
			$cms_log->info('user', 'clients_rename_lang', array('idclient' => $collapse, 'clientname' => $clientname, 'idlang' => $idlang, 'name' => $newname));
		}
		// Event
		fire_event('clients_rename_lang', array(
			'idclient' => $collapse,
			'clientname' => $clientname,
			'idlang' => $idlang,
			'name' => $newname,
			'description' => $newdesc,
			'charset' => $charset,
			'rewrite_key' => $_REQUEST['rewrite_key'],
			'rewrite_mapping' => $_REQUEST['rewrite_mapping'],
			'author' => $auth->auth['uid'],
			'errno' => $errlog
		));
		break;
	case 'delete_lang':  // Sprache löschen
		...
		// Event
		fire_event('clients_delete_lang', array(
			'idclient' => $collapse,
			'clientname' => $clientname,
			'idlang' => $lid,
			'name' => $langname,
	...
	case 'makestartlang':
		...
		// Event
		fire_event('clients_make_start_lang', array(
			'idclient' => (int)$_REQUEST['cid'],
			'clientname' => $clientname,
			'idlang' => (int)$_REQUEST['lid'],
			'name' => $langname,
			'author' => $auth->auth['uid'],
			'errno' => $errlog
		));
...
{	
	...
	$tpl_data['LANG_DESCFIELD'] = $cms_lang['clients_client_desc']. ': ';
	$tpl_data['ENTRY_DESCFIELD'] = sprintf($c_conf['html_field_text'], 'newdesc', $client_array[$idclient]['langs'][$idlang]['desc'], 300, 150);
	$tpl_data['LANG_CHARSET'] = $cms_lang['clients_client_charset'].': ';
	$tpl_data['ENTRY_CHARSET'] = clients_get_charset_selectbox($client_array[$idclient]['langs'][$idlang]['charset']);
	$tpl_data['LANG_ISO_3166'] = $cms_lang['clients_client_auto_startlang'].': ';
	$tpl_data['LANG_REWRITE_KEY'] = $cms_lang['clients_client_prefix_rewrite'].': ';
	...
	$tpl_data['LANG_REWRITE_MAPPING'] = $cms_lang['clients_client_urlfilter'].': ';
...
{	
	...
	$newpath = ($newpath == '') ? $cfg_cms['path_base']."projekt0".$nextclient."/" : $newpath;
	$newurl = ($newurl == '') ? $cfg_client['path_http']."projekt0".$nextclient."/" : $newurl;
