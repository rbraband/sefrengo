<?php
// File: $Id: fnc.con.php 619 2012-01-27 21:27:34Z holger $
// +----------------------------------------------------------------------+
...
function con_lock ($type, $id, $visible) {
	...
	switch ($type) {
		...
		case 'cat':
			...
			// entsperren / sperren aller Ordner unabh�ngig von der Sprache
			...
			//pr�fen ob sides vorhanden, bei leerer Kategorie nicht der fall
		...
		case 'side':
			...
			// entsperren / sperren aller Seiten unabh�ngig von der Sprache
...
function con_delete_cat ($idcat) {
	...
	if ($db->next_record()) return '0201';
		...
		if ($db->next_record()) return '0202';
			...
			// Eintr�ge aus der cat_lang Tabelle l�schen
...
// change JB - sprache ist nicht mehr Pflicht ... f�r sperren von Seiten notwendig
...
function con_delete_side ($idcat, $idside) {
	...
	// Event
	fire_event('delete_side', array ('idside' => $idside, 'idcat' => $idcat, 'idcatsides' => $idcatsides));

	// aus 'code'-Tabelle l�schen
	...
	if (is_array($idsidelang)) {
		foreach ($idsidelang AS $key => $value) {
			...
			// Links, die auf diese Seite zeigen l�schen
			// Seitenstatus dieser Seiten auf ge�ndert stellen
            // mu� noch gemacht werden
		}
	}

	// Seite aus den Ordnern l�schen
	...
	// Seite aus den verschiedenen Sprachen l�schen
...
function con_expand ($idcat, $expand_action) {
	...
	switch($expand_action) {
		...
		case '3':
			...
			// alles l�schen, damit doppelte Datens�tze vermieden werden
			$sql = "DELETE FROM $cms_db[cat_expand] WHERE idusers = '$user_id' AND idcat IN(".implode(',', $list).")";
            $db->query($sql);
...
function con_publish ($id, $type='all') {
	...
	if ($view) {
		...
		header ('Location:'.$sess->urlRaw($cfg_client['path_http'] . $cfg_client['path_rel'].$cfg_client['contentfile'].'?lang='.$lang.'&idcatside='.$idcatside.'&view='.$view));
		exit;
	}
}
...
function con_create_site_meta_from_idcatside($idclient, $idlang_current, $idcatside_from, $name='', $lang_postfix= true, $target_idcat=-1, $options =array()) {
	...
	//get lang infos
	include_once $cfg_cms['path_base'].$cfg_cms['path_backend_rel']."inc/fnc.clients.php";
