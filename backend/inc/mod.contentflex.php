...
//API
$cf_modinfo = sf_api('LIB', 'Modinfo');
$cf_req = sf_api('LIB', 'WebRequest');
...
{
	function edit_outer_tpl($last, $now, $mvars)
	{
        if (($last == '') && ($now == '')) return '';
...
if(isset($page)) {
    $page = (! is_numeric($cf_req->req('page'))) ? 1:$cf_req->req('page');
} else {
    $page = 1;
}
...
if(isset($items)) {
    $mvars['max_items'] = (! is_numeric($cf_req->req('items'))) ? 1:$cf_req->req('items');
}
...
#}
#
#if ((bool) $cf_modinfo->isLastEntry()){
...
if($cf_modinfo->getEntryNr() >= $page
&& ($cf_modinfo->getEntryNr() < ($mvars['max_items']+$page)
||  $mvars['max_items'] == 0)) {
    ...
    // **** Umschlie�endes Template  *************************************************************
    if ( !isset(${'flex2_outer_tpl'.$cms_mod['container']['id']}) || $cf_modinfo->getEntryNr() == 1) {
        ${'flex2_outer_tpl'.$cms_mod['container']['id']} = '';
    }
    $mvars['tpl_outer'] = stripslashes($mvars['tpl_outer']);
    $temp = edit_outer_tpl(${'flex2_outer_tpl'.$cms_mod['container']['id']}, $mvars['tpl_outer'], $mvars);
...
//   	if($mvars['is_last'] == true || ($mvars['repeat_id'] == ($mvars['max_items']+$page-1))) {
        $mvars['tpl_inner'] .= edit_outer_tpl(${'flex2_outer_tpl'.$cms_mod['container']['id']}, '', $mvars) ;
    }
    
    
    // **** Ende Umschlie�endes Template  ********************************************************
