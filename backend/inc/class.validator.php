<?PHP
// File: $Id: class.validator.php 619 2012-01-27 21:27:34Z holger $
// +----------------------------------------------------------------------+
...
//		$name = preg_replace('/[ÄÆ]/u', 'Ae', $name);
	...
	function _css_element($name, $evalue) {
		...
		if (!$this->css_units)
		{
			//$this->css_units    = $val_ct->get_by_group('css_units', 0);
			$valcol = sf_api('MODEL', 'ValueSqlCollection');
			$valcol->getByGroup('css_units');
			$this->css_units = $valcol->getAssocKeyArray();
			unset($valcol);
		}
		if (!$this->css_elements)
		{
			//$this->css_elements = $val_ct->get_by_group('css_elements', 0);
			$valcol = sf_api('MODEL', 'ValueSqlCollection');
			$valcol->getByGroup('css_elements');
			$this->css_elements = $valcol->getAssocKeyArray();
			unset($valcol);
		}
