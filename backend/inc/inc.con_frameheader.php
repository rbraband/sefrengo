<?PHP
// File: $Id: inc.con_frameheader.php 641 2012-02-18 02:33:32Z bjoern $
// +----------------------------------------------------------------------+
...
<div id="main">
	...
	function sf_setCurrentIdcatside(idcatside, is_disabled, idlang) {
		...
		try {
			...
			if (idlang != sel.value) {
				is_disabled = true
				window.location = '<?php echo $sess->urlRaw($cfg_cms['path_base_http'] . $cfg_cms['path_backend_rel'] . 'main.php?place=holder') ?>&idcatside='+idcatside+'&area=con_frameheader&unfreeze=true&lang=' + idlang
			}
	...
	function sf_confirmContentcopy(lang) {
		return confirm("<?php echo $cms_lang['gen_copy_pagecontent_from_lang'] ?> '"+ lang
						+ "' <?php echo $cms_lang['gen_copy_pagecontent_from_lang2'] ?>"+"\n"
						+ "<?php echo $cms_lang['gen_copy_pagecontent_from_lang3'] ?>\n"
						+ "<?php echo $cms_lang['gen_copy_pagecontent_from_lang4'] ?>");
	}
	</script>
