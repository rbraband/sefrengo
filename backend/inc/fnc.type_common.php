<?PHP
// File: $Id: fnc.type_common.php 685 2012-04-11 00:49:36Z bjoern $
// +----------------------------------------------------------------------+
 ...
 * @Access private
 */
function _type_check_editable($tag_attribute)
{

	$cfg = sf_api('LIB', 'Config');
    

	if ($cfg->env('view') != 'edit')
	{
		return FALSE;
	}
	
	if (! $cfg->env('perm_edit_page'))
	{
		return FALSE;
	}

	if (! $cfg->env('current_container_editable'))
	{
		return FALSE;
	}

	if (! _type_check_local_tag_perm($tag_attribute))
	{
		return FALSE;
	}
...
function _type_get_stylesheet() {
	...
	if (!$cache['stylesheet']) {
		...
		while ($db->next_record()) $cache['stylesheet_files'] .= "<link rel=\"StyleSheet\" href=\"".$cfg_client['path_http'] . $cfg_client['path_rel'].$db->f('dirname').$db->f('filename')."\" type=\"text/css\">";
 ...
 *               [$i]['dirname'] - full directory relative from $sf_cfg->env('path_frontend_fm')
 *               [$i]['filename'] - filename
...
function _type_get_layer_menu($ids, $infos, $delete, $new, $up, $down, $inline=false)
{
	...
	if($inline)
		$p_menu->add_entry(
			$cms_lang['type_save'], $infos['base_url'], '_self', $cms_lang['type_save'],
	...
	if($infos['mode'] == 'advanced'){
		...
		if($new)
			$p_menu->add_entry(
				$cms_lang['type_new'], $infos['base_url'].'&action=new&entry='.$infos['mod_repeat_id'].'&content='
				.$infos['container_number'].'.new.'.$subs_to_string, '_self', $cms_lang['type_new']);
		if($delete)
			$p_menu->add_entry(
				$cms_lang['type_delete'], $infos['base_url'].'&action=delete&content='
				.$infos['container_number'].'.'.$infos['mod_repeat_id'], '_self', $cms_lang['type_delete'], 'if(delete_confirm())');
		if($up)
			$p_menu->add_entry(
				$cms_lang['type_up'], $infos['base_url'].'&action=move_up&content='
				.$infos['container_number'].'.'.$infos['mod_repeat_id'], '_self', $cms_lang['type_up']);
		if($down)
			$p_menu->add_entry(
				$cms_lang['type_down'], $infos['base_url'].'&action=move_down&content='
				.$infos['container_number'].'.'.$infos['mod_repeat_id'], '_self', $cms_lang['type_down']);
	}
