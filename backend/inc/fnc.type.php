<?PHP
// File: $Id: fnc.type.php 685 2012-04-11 00:49:36Z bjoern $
// +----------------------------------------------------------------------+
...
{
	global $sess, $cfg_cms, $cfg_client, $idcatside, $cms_lang, $content, $cms_edittype;
	...
	// Editiermodus
	if (_type_check_editable($type_config['editable']) && $type_config['menuoptions'] != 'false') {

		$title = (empty($type_config['title'])) ? $cms_lang['type_text'] : $type_config['title'];
		// advanced Modus
...
function type_output_textarea($type_container, $type_number, $type_typenumber, $type_config) {
	global $sess, $cfg_cms, $cfg_client, $idcatside, $cms_lang, $content;
	...
	if (_type_check_editable($type_config['editable']) && $type_config['menuoptions'] != 'false')
	{
		...
		$title = (empty($type_config['title'])) ? $cms_lang['type_textarea'] : $type_config['title'];
...
{
	global $sess, $cfg_cms, $cfg_client, $idcatside, $cms_lang, $content, $sid_sniffer;
...
function _type_output_wysiwyg_common($type_container, $type_number, $type_typenumber, $type_config, $unique_type_id)
{
	...
	if (_type_check_editable($type_config['editable']) && $type_config['menuoptions'] != 'false')
	{
		  ...
		  $title = (empty($type_config['title'])) ? $cms_lang['type_wysiwyg'] : $type_config['title'];
		...
		if ($tmp_sniffer['18'] == 'true' || $tmp_sniffer['9'] == 'true') { 
			...
			//Mozilla braucht midestens ein "&nbsp;" um auf ein editdiv zugreifen zu k�nnen/damit dort das editieren funktioniert
...
function type_output_link($type_container, $type_number, $type_typenumber, $type_config)
{
	...
	if (_type_check_editable($type_config['editable']) && $type_config['menuoptions'] != 'false')
	{
		...
		$title = (empty($type_config['title'])) ? $cms_lang['type_link'] : $type_config['title'];
...
function type_output_file($type_container, $type_number, $type_typenumber, $type_config)
{
	...
	if (preg_match_all('#^cms://(idfile|idfilethumb)=(\d+)$#', $file_id, $match) ) {
		...
		if($type_config['mode'] == 'id') return $id;
                ...
                $file_obj = sf_api('MODEL', 'FileSqlItem');
                $file_obj->setIdclient($sf_cfg->env('idclient'));
                $file_obj->setIdlang($sf_cfg->env('idlang'));
	

		if ($file_obj->loadById($id)) {			
			$file_url = $sf_cfg->env('path_frontend_fm_http').$file_obj->getRelativePath();
			$file_path = $sf_cfg->env('path_frontend_fm').$file_obj->getRelativePath();
			$file_size = $file_obj->getField('filesize');
			$file_db_desc  = $file_obj->getField('description');
			$file_db_titel = $file_obj->getField('title');
			$file_filetype = $file_obj->getField('filetype');
			$file_name     = $file_obj->getField('filename');
			
			if ($is_thumb){
                            $thumb_arr = $file_obj->getThumbnails(0);
                            $file_url = $thumb_arr['html_path'];
                            $file_path = $thumb_arr['path'];
			}
		}
	}
	...
	if (_type_check_editable($type_config['editable']) && $type_config['menuoptions'] != 'false')
	{
		...
		$title = (empty($type_config['title'])) ? $cms_lang['type_file'] : $type_config['title'];
...
function type_output_sourcecode($type_container, $type_number, $type_typenumber, $type_config)
{
	...
	if (_type_check_editable($type_config['editable']) && $type_config['menuoptions'] != 'false')
	{
		$title = (empty($type_config['title'])) ? $cms_lang['type_sourcecode'] : $type_config['title'];
...
function type_output_typegroup($type_container, $type_number, $type_typenumber, $type_config)
{
	...
	if (_type_check_editable($type_config['editable']) && $type_config['menuoptions'] != 'false')
	{
		...
		$title = ( empty($type_config['title']) ) ? $cms_lang['type_typegroup'] : $type_config['title'];
		// advanced Modus
		if ($type_config['menuoptions'] == 'advanced') {
			$new=false; $delete=false; $up= false; $down=false;
			// neu anlegen & loeschen
...
function type_output_select($type_container, $type_number, $type_typenumber, $type_config) {
    global $sess, $cfg_cms, $cfg_client, $idcatside, $cms_lang, $content, $cms_edittype, $cms_mod, $con_side;
    ...
    if (_type_check_editable($type_config['editable']) && $type_config['menuoptions'] != 'false')
	{
        ...
        $title = (empty($type_config['title'])) ? $cms_lang['type_select'] : $type_config['title'];
...
{
    global $sess, $cfg_cms, $cfg_client, $idcatside, $cms_lang, $content, $cms_edittype, $con_side;
...
function type_output_checkbox($type_container, $type_number, $type_typenumber, $type_config) {
	...
	if (_type_check_editable($type_config['editable']) && $type_config['menuoptions'] != 'false')
	{
		...
		$title = (empty($type_config['title'])) ? $cms_lang['type_checkbox'] : $type_config['title'];
...
function type_output_radio($type_container, $type_number, $type_typenumber, $type_config) {
	...
	if (_type_check_editable($type_config['editable']) && $type_config['menuoptions'] != 'false')
	{
		...
		$title = (empty($type_config['title'])) ? $cms_lang['type_radio'] : $type_config['title'];
...
function type_output_date($type_container, $type_number, $type_typenumber, $type_config) {
    ...
    if ((empty($mod_content)) || ($mod_content=="")) {
        ...
        if ((empty($type_config['mode'])) || ($type_config['mode'] == 'default-cms-format')) $mod_content = date($cfg_cms['format_date'].' '.$cfg_cms['format_time'],$mod_content);
        elseif ((empty($type_config['mode'])) || ($type_config['mode'] == 'default-cms-date-format')) $mod_content = date($cfg_cms['format_date'],$mod_content);
        elseif ((empty($type_config['mode'])) || ($type_config['mode'] == 'default-cms-time-format')) $mod_content = date($cfg_cms['format_time'],$mod_content);
	...
	if (_type_check_editable($type_config['editable']) && $type_config['menuoptions'] != 'false')
	{
		...
		$title = (empty($type_config['title'])) ? $cms_lang['type_date'] : $type_config['title'];
