<?php
// File: $Id: class.SF_GUI_RESSOURCES_FileManager.php 74 2008-11-25 21:16:29Z bjoern $
// +----------------------------------------------------------------------+
...
class SF_GUI_RESSOURCES_FileManager extends SF_GUI_RESSOURCES_Abstract{
	...
	function generateCatData($current_position) 
	{
			...
			//MAKE CATS
			$sql = "SELECT DISTINCT C.*, E.description, D.name AS firstname, D.surname, D.username
				FROM ".$cms_db['directory']." C
				LEFT JOIN ".$cms_db['directory_lang']." E USING(iddirectory),
				".$cms_db['users']." D
				WHERE C.idclient= '$client'
					AND C.lastmodified_author = D.user_id
	...
	function _generateItems($type, $current_position, $item_position =null) 
	{
		...
		$date_format = $cfg_cms['format_date'] . ' ' . $cfg_cms['format_time'];
		$sql = "SELECT
				A.*, A.created AS fcreated, A.lastmodified AS flastmodified, B.filetype, C.dirname, C.iddirectory, D.*, E.title, E.description
			FROM
				".$cms_db['upl']." A
				LEFT JOIN ".$cms_db['filetype']." B USING(idfiletype)
				LEFT JOIN ".$cms_db['directory']." C ON A.iddirectory=C.iddirectory
				LEFT JOIN ".$cms_db['upl_lang']." E USING(idupl),
				".$cms_db['users']." D
			WHERE
				A.idclient= '$client'
				AND E.idlang= '$lang'
				AND A.lastmodified_author = D.user_id
				AND C.dirname NOT LIKE('cms/%')
				$sql_filetypes
				$sql_format
		...
		while ( $db->next_record() )
		{
			...
			$configdata = unserialize(stripslashes($db->f('configdata')));
			$thumbdata = array();
			if(is_array($configdata) && array_key_exists('thumbs', $configdata))
			{
				$thumbdata = $configdata['thumbs'];
			}
			
			//create thumbnaill if possible
			if (in_array($pic_filetype, $thumb_types)) {
				
				if (! file_exists($sf_cfg->env('path_frontend_fm').$db->f('dirname').$t_name) ) {
					if ($db->f('pictwidth') >0 && $db->f('pictheight')>0 ) { 
						...
						$thumb = sprintf($thumb_stencil,  $sf_cfg->env('path_frontend_fm_http').$db->f('dirname').$db->f('filename'),
					...
					// make thumb
					$thumb = sprintf($thumb_stencil,  $sf_cfg->env('path_frontend_fm_http').$db->f('dirname').$t_name,
			...
			if($type == 'format') 
			{
				...
				$picker_val = ($this->custom_config['return_value_mode'] == 'sefrengolink') 
								...
								$sf_cfg->env('path_frontend_fm_http').$db->f('dirname').$db->f('filename');
				$this->_addVariousFormatItem($db->f('filename'), $picker_val, 
								array(  'title_after' => '',
										'description' => $this->lang['rb_res_file_picformat'] .': '.$picsize,
				...
				foreach($thumbdata as $thumb_num => $thumbdata)
				{
					$t_name = substr ($pic_filename, 0, ($name_length - $extension_length - 1) );
					$t_name .= $cfg_client['thumbext'];
					$t_name .= ($thumb_num > 0) ? '_'.$thumb_num : '';
					$t_name .= '.'. $pic_filetype;	
					
					$picker_val = ($this->custom_config['return_value_mode'] == 'sefrengolink') 
									? 'cms://idfilethumb='. $db->f('idupl').'?thumb='.$thumb_num:
									$sf_cfg->env('path_frontend_fm_http').$db->f('dirname').$t_name;
					$this->_addVariousFormatItem($t_name, $picker_val, 
									array(  'title_after' => '',
											'description' => $this->lang['rb_res_file_picformat'] .': '.$thumbdata['width'].'x'.$thumbdata['height'].' px',
											'detail_description1' => $thumb,
											'detail_description2' => '<strong>'.$this->lang['rb_res_file_filewidth'].': </strong>'.$thumbdata['width'].' px<br />',
											'detail_description3' => '<strong>'.$this->lang['rb_res_file_fileheight'].': </strong>'.$thumbdata['height'].' px<br />',
											'detail_description4' => '',
											'detail_description5' => ''
											), 
									$db->f('filetype'), true, false, $db->f('dirname'));
				...
				if ($item_position) {
					$title_after = '<span style="font-size:9px;color:#5A7BAD;" title="'.$this->lang['rb_res_file_mutiple_picsizes'].'"> ('.(count($thumbdata)+1).')</span>';
				} else {
					$title_after = '';
				}
				$picker_val = ($this->custom_config['return_value_mode'] == 'sefrengolink') 
				...
				$this->_addItem($db->f('filename'), $picker_val, 
									array( 'title_after' => $title_after,
											'description' => $db->f('title'),
											'detail_description1' => $thumb,
											'detail_description2' => '<strong>'.$this->lang['rb_title'].': </strong>'.$db->f('title').'<br />',
