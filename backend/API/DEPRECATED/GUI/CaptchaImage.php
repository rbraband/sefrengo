<?php
class SF_GUI_CaptchaImage extends SF_LIB_ApiObject {
	...
	function SF_GUI_CaptchaImage() {
		...
		include_once $cfg_cms['path_base'].$cfg_cms['path_backend_rel'].'external/misc/class.captcha.php';
		$this->captcha_object = new captcha();
		$this->picture_path = $cfg_cms['path_base'] . $cfg_client['path_rel'].'cms/files/tmp/captcha';
		if (! is_dir($cfg_cms['path_base'] . $cfg_client['path_rel'].'cms/files/tmp')) {
			mkdir ($cfg_cms['path_base'] . $cfg_client['path_rel'].'cms/files/tmp', 0777);
		}
		if (! is_dir($cfg_cms['path_base'] . $cfg_client['path_rel'].'cms/files/tmp/captcha')) {
			mkdir ($cfg_cms['path_base'] . $cfg_client['path_rel'].'cms/files/tmp/captcha', 0777);
		}
	}
