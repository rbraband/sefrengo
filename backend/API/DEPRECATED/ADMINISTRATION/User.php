<?php
class SF_ADMINISTRATION_User extends SF_LIB_ApiObject {
	...
	function getHaveFrontendAccess() {
		global $cms_db;
	...
	function save() {
		...
		if ($this->dirty) {
			if ($this->data['sql']['users']['user_id'] > 0) {
				...
				//Log
				$cms_log->info('user', 'user_update', array('iduser' => $this->getIduser(), 'username' => $this->getUsername()));
				//Event
				...
				//Log
				$cms_log->info('user', 'user_create', array('iduser' => $this->getIduser(), 'username' => $this->getUsername()));
				//Event
	...
	function delete() {
		...
		//Log
		$cms_log->info('user', 'user_delete', array('iduser' => $this->getIduser(), 'username' => $this->getUsername()));
		//Event
	...
	function loginToBackend()
	{
		...
		$sess->start();
		$auth->auth['uid'] = $this->getIduser();
		$auth->auth['uname'] = $this->getUsername();
	...
	function loginToFrontend()
	{
		if (! $this->getHaveFrontendAccess())
		{
			return FALSE;
		}
