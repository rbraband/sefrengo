...
class SF_MODEL_FileSqlCollection extends SF_MODEL_AbstractSqlCollection
{
	/**
	 * Constructor sets up the model and fulltext searchfields
	 * @return void
	 */
	public function __construct()
	{
		...
		$this->setCfg('perm_type', 'file');
		$this->setCfg('perm_nr', '17');
		$this->setCfg('perm_dbfield_id', $this->tables[0].'.idupl');
		$this->setCfg('perm_dbfield_parent', $this->tables[0].'.iddirectory');
		
		// set fulltextsearchfields afterwards
		$this->setCfg(
			'fulltextsearchfileds', 
			array(
				$this->tables[0].'.filename',
				$this->tables[1].'.title',
	...
	private function _getLeftJoinSql()
	{
		$sql = "
				LEFT JOIN
					".$this->tables[1]."
				ON(".
					$this->_getMappedIdField($this->tables[0])." = ".$this->tables[1].".idupl"
				.")";
		return $sql;
	}
}
