<?php
// File: $Id: local.php 595 2011-12-18 22:15:26Z bjoern $
// +----------------------------------------------------------------------+
...
class DB_cms extends DB_Sql {
    ...
    function haltmsg( $msg ) {
		...
		$cms_log->error('sql', 'MySql-Error:' . $msg);
    } 
    ...
    // free result
    function free() {
    	// if statement prevent php warning for mysql_free_result
    	if($this->Query_ID != 1 && $this->Query_ID != 0) {
    ...
    function query( $Query_String, $Cache_it = 0, $Cache_var = '' ) {
        ...
        if( $Cache_it >= 1 && $this->use_cache ) {
            ...
            if ( $this->read_cache( $cache ) ) {
                ...
                $this->Row = 0;
                if ( $this->Debug ) {
					$cms_log->debug('sql', 'Cache: ' . $Query_String);
                } 
                return ( $this->Query_ID = 1 );
            } 
        } 
        if ( $this->Debug ) {
			$cms_log->debug('sql', 'MySql: ' . $Query_String);
        } 
...
class cms_Backend_Auth extends Auth {
    ...
    function auth_loginform() {
        global $cfg_cms, $cfg_client, $lang, $sess;

        if ( array_key_exists('view', $_REQUEST) ) {
    			$hd = sf_api('LIB', 'HttpHeader');
    			$hd->redirect($sess->url($cfg_cms['path_base_http'] . $cfg_cms['path_backend_rel'].'main.php'));
        } else {
    ...
    function auth_validatelogin() {
        ...
        // Event
        fire_event( 'login_fail_input', array ( 'username' => $username, 'password' => $password ) );
        return false;
    }
